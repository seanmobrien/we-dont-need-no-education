...
public class SpanChatModelListener implements ChatModelListener {
  ...
  public void onRequest(ChatModelRequestContext requestContext) {
    ...
    SpanBuilder spanBuilder = tracer
      .spanBuilder("chat " + request.parameters().modelName())
    ...
    if (request.parameters().temperature() != null) spanBuilder.setAttribute(
      "gen_ai.request.temperature",
      request.parameters().temperature()
    );

    if (request.parameters().topK() != null) spanBuilder.setAttribute(
      "gen_ai.request.top_k",
      request.parameters().topK()
    );

    if (request.parameters().topP() != null) spanBuilder.setAttribute(
      "gen_ai.request.top_p",
      request.parameters().topP()
    );

    if (
      request.parameters().presencePenalty() != null
    ) spanBuilder.setAttribute(
    ...
    if (
      request.parameters().frequencyPenalty() != null
    ) spanBuilder.setAttribute(
      "gen_ai.request.frequency_penalty",
      request.parameters().frequencyPenalty()
    );

    if (request.parameters().stopSequences() != null) spanBuilder.setAttribute(
  ...
  public void onResponse(ChatModelResponseContext responseContext) {
    ...
    if (span != null) {
      ...
      span
        .setAttribute("gen_ai.response.id", response.metadata().id())
