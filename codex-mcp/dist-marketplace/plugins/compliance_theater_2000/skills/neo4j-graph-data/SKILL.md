...
description: Use the Compliance Theater Neo4j graph for case-file, policy, evidence, email-thread, actor, attachment, subject-matter, embedding, and relationship traversal questions. Trigger when a user asks what graph data exists, how to query Neo4j, how case-file documents connect to policy or people, how to traverse evidence relationships, or how to use graph_read, graph_schema, graph_embed, or Cypher against Compliance Theater data.
---

# Neo4j Graph Data

Use this skill to reason over the Compliance Theater Neo4j graph: case-file documents, document properties, email context, actors, policy references, subject-matter concepts, embeddings, and graph-maintenance runs.
...
6. For vector work, use `vectorParams` or inline `{ "$embed": "...", "modelSize": "small" }` parameters rather than pasting vectors. Default to `small` for the 1536-dimension `text-embedding-3-small` graph embeddings.
7. Cite graph tool use and important IDs in the answer: case-file IDs, policy keys, actor names, thread IDs, or relationship types.
