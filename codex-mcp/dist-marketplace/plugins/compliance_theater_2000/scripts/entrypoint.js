...
const local_file_resources_1 = require("./local-file-resources");
...
function exposedHelperToolsForToolset() {
    const toolset = (0, config_1.configuredToolset)();
    if (toolset === "memory") {
...
function namespaceNamesForToolset() {
    const toolset = (0, config_1.configuredToolset)();
    if (toolset === "all") {
        return serverNames;
    }
...
function listOpenAiNamespaceTools() {
...
async function callHelperTool(id, name, args = {}) {
    try {
        ...
        if (toolset === "memory" && memoryTools.some((tool) => tool.name === localName)) {
            sendToClient({ jsonrpc: "2.0", id, result: jsonToolResult(await (0, app_tools_1.callMemoryTool)(localName, args, freshTokenAfterBadRequest)) });
            return;
        }
        if (localName === "read_case_file") {
            sendToClient({ jsonrpc: "2.0", id, result: jsonToolResult(await (0, app_tools_1.readCaseFileTool)(args, freshTokenAfterBadRequest)) });
            return;
        }
        if (localName === "get") {
            sendToClient({ jsonrpc: "2.0", id, result: jsonToolResult(await (0, app_tools_1.getCaseFileTool)(args, remoteRequest, freshTokenAfterBadRequest)) });
            return;
        }
        if (localName === "amend") {
            sendToClient({ jsonrpc: "2.0", id, result: jsonToolResult(await (0, app_tools_1.amendCaseFileTool)(args, remoteRequest)) });
            return;
        }
        if (localName === "embed") {
            sendToClient({ jsonrpc: "2.0", id, result: jsonToolResult(await (0, app_tools_1.manageCaseFileEmbeddingsTool)(args, freshTokenAfterBadRequest)) });
            return;
        }
        ...
        if (localName === "call_api") {
            sendToClient({ jsonrpc: "2.0", id, result: jsonToolResult(await (0, app_tools_1.callApiTool)(args, freshTokenAfterBadRequest)) });
            return;
        }
        ...
        if (localName === "auth") {
            if (args?.action === "clear-cache") {
                ...
                sendToClient({ jsonrpc: "2.0", id, result: textToolResult(await (0, auth_1.authStatusSummary)()) });
            }
            else if (args?.action === "login") {
                sendToClient({ jsonrpc: "2.0", id, result: textToolResult(await (0, auth_1.loginAndSummarizeStatus)()) });
            }
            else {
...
async function handleClientRequest(message) {
    ...
    if (message.method === "resources/read" && ((0, config_1.configuredToolset)() === "default" || (0, config_1.configuredToolset)() === "all")) {
        const localResource = await (0, local_file_resources_1.readLocalFileResource)(String(message.params?.uri || ""));
        if (localResource) {
