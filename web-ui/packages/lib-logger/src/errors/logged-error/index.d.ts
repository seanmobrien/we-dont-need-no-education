 ...
 * - Integration with error reporting services via event emitter pattern
 ...
 * - Integration with error reporting services via event emitter
 * - Stack trace preservation and enhancement
 ...
 *
 * **IMPORTANT**: The only interface into the error reporting system is the event emitter.
 * There are no direct references to error monitoring systems outside of emitting events.
 * External systems subscribe to error events via `subscribeToErrorReports()`.
 *
 * @class LoggedError
 * @implements {Error}
 *
...
export declare class LoggedError extends Error {
  /**
   * Subscribe to error report events.
   * This is the only interface into the error reporting system.
   * External error monitoring systems should use this to receive error events.
   *
  ...
  /**
   * Unsubscribe from error report events.
   *
   * @param callback - The same callback function passed to subscribeToErrorReports
   */
  ...
  /**
   * Clear all error report subscriptions.
   ...
   * - Integration with error reporting services via event emission
   ...
   *
   * @param {TurtleRecursionParams} [options] - Configuration options for error handling.
   ...
   *   with all specified logging and reporting completed via event emission.
   *
   * @example
   * ```typescript
 ...
 * @since 1.0.0
 */
export declare function dumpError(e: unknown): string;
