...
const uiOnly = (
  ...
  if (typeof window !== 'undefined' && !isRunningOnEdge()) {
    ...
    return () =>
      log((l) =>
        l.info(
          `UI-only action ${recoveryAction.label} skipped in ${typeof window === 'undefined' ? 'server' : 'edge'
          } environment.`
        )
      );
  }
...
// store action wrappers for navigation service methods
const clientReload = uiOnly('refresh-page', () => window.location.reload());
const clientNavigateSignIn = uiOnly('login-redirect', () => {
