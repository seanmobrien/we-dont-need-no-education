...
import {
  ...
  ErrorType,
  recoveryStrategies,
} from '../../src/errors/monitoring/recovery-strategies';
import {
  asValue,
  registerServices,
...
const registerNavigationService = () => {
  registerServices({
    clientNavigate: asValue({
  ...
  describe('getDefaultRecoveryAction', () => {
    ...
    it('should return first action if no default is specified', async () => {
      ...
      if (strategyWithoutDefault) {
        ...
        jest
          .spyOn(recoveryModule, 'classifyError')
...
describe('Recovery Strategy Configuration', () => {
  ...
  it('should only allow authentication as a duplicated strategy type', () => {
    const errorTypes = recoveryStrategies.map((s) => s.errorType);
    const duplicateTypes = errorTypes.filter(
      (errorType, index) => errorTypes.indexOf(errorType) !== index,
    );

    expect(duplicateTypes).toEqual([ErrorType.AUTHENTICATION]);
  });
