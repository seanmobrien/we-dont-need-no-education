...
} from '../../src/errors/monitoring';
import { useErrorReporter } from '../../src/errors/monitoring/use-error-reporter';

jest.mock('../../src/errors/monitoring/error-reporter', () => {
  const originalModule = jest.requireActual(
    '../../src/errors/monitoring/error-reporter',
  );
  ...
  const mockErrorReporter = jest.fn(
    (cb?: (r: typeof mockReporterInstance) => unknown) => {
      if (cb) {
...
describe('useErrorReporter', () => {
  ...
  describe('Hook Stability', () => {
    it('should not cause infinite re-renders when used in useEffect', async () => {
      ...
      expect(renderCount).toBe(4); // Includes React's additional effect cycle in test env
    });
  });
