...
export const KeyRefreshNotify = () => {
  ...
  const [lastNotifiedSessionStatus, setLastNotifiedSessionStatus] =
    useState<string>("");
  const [lastNotifiedKeyStatus, setLastNotifiedKeyStatus] =
    useState<string>("");
  const [hasShownInitialLoadingToast, setHasShownInitialLoadingToast] =
    useState(false);
  ...
  useEffect(() => {
    ...
    if (sessionStatus === "loading" && !hasShownInitialLoadingToast) {
      ...
      lastNotifiedSessionStatus === "loading"
    ) {
    ...
    sessionStatus,
    notifications,
    hasShownInitialLoadingToast,
    lastNotifiedSessionStatus,
  ]);
  ...
  useEffect(() => {
    ...
    switch (keyStatus) {
      ...
      case "invalid":
        ...
        notifications.show(
          "Authentication keys are out of date and being refreshed...",
          {
            severity: "info",
      ...
      case "failed": {
        ...
        const errorMessage =
          error || "Key validation failed. Please try again later.";
      ...
      case "synchronized":
        ...
        notifications.show(
          "Authentication keys have been successfully refreshed.",
          {
            severity: "success",
            key: NOTIFICATION_KEY_KEY_VALIDATION,
