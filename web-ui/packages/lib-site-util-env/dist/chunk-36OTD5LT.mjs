...
var serverEnvSchema = z.object({
  AUTH_SECRET: z.string().min(1).describe("auth.js secret used when signing tokens"),
  ...
  AZURE_API_KEY: z.string().min(1).describe(
  ...
  GOOGLE_GENERATIVE_AI_API_KEY: z.string().min(1).optional().describe(
  ...
  AUTH_KEYCLOAK_SCOPE: z.string().optional().default("openid").describe(
    "Keycloak OAuth scope for authentication (optional). Example: openid mcp_tool"
  ),
  AUTH_KEYCLOAK_IMPERSONATION_AUDIENCE: z.string().optional().describe(
    "Keycloak audience for user impersonation tokens (optional). Example: admin-cli"
  ),
  AUTH_KEYCLOAK_REDIRECT_URI: z.string().describe(
  ...
  AUTH_KEYCLOAK_IMPERSONATOR_USERNAME: z.string().optional().describe(
    "Keycloak impersonator username for user impersonation tokens (optional). Example: admin"
  ),
  AUTH_KEYCLOAK_IMPERSONATOR_PASSWORD: z.string().optional().describe(
    "Keycloak impersonator password for user impersonation tokens (optional). Example: admin"
  ),
  AUTH_KEYCLOAK_IMPERSONATOR_OFFLINE_TOKEN: z.string().optional().describe(
    "Keycloak impersonator offline token for user impersonation tokens (optional). Example: admin"
  ),
  AUTH_KEYCLOAK_MCP_TOOL_RESOURCE_NAME: z.string().describe(
  ...
  AUTH_KEYCLOAK_MCP_TOOL_RESOURCE_ID: z.string().describe(
  ...
  MEM0_API_BASE_PATH: z.string().default("api/v1").describe(
  ...
  MEM0_USERNAME: z.string().min(1).describe(
    "Mem0 service username for authentication. Example: user@example.com"
  ),
  ...
  MEM0_API_KEY: z.string().optional().describe(
    "Mem0 API key for service authentication. Example: mem0_sk_1234567890abcdef..."
  ),
  NODE_ENV: z.string().describe(
