...
const uiOnly = (action, doIt) => {
    const recoveryAction = typeof action === 'string'
        ? {
            id: action,
    ...
    if (typeof window !== 'undefined' && !isRunningOnEdge()) {
        ...
        return () => log((l) => l.info(`UI-only action ${recoveryAction.label} skipped in ${typeof window === 'undefined' ? 'server' : 'edge'} environment.`));
    }
};
    ...
    },
    {
        ...
        actions: [
            ...
            },
            {
                ...
                action: uiOnly('reduce-requests', () => alert('Rate limit exceeded. Please slow down your requests and try again in a minute.')),
            },
        ],
...
export function getDefaultRecoveryAction(error) {
    const errorType = classifyError(error);
    const strategy = recoveryStrategies.find((s) => s.errorType === errorType);
    if (!strategy)
        return null;
    ...
    if (!defaultActionId)
        return strategy.actions[0] || null;
    return strategy.actions.find((a) => a.id === defaultActionId) || null;
}
export async function attemptAutoRecovery(error) {
