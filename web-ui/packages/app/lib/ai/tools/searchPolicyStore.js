...
import { hybridPolicySearchFactory, } from '../services/search';
...
const searchPolicyStoreCounter = appMeters.createCounter('ai_tool_search_policy_store_total', {
    description: 'Total number of policy store search operations',
    unit: '1',
});
const searchPolicyStoreDurationHistogram = appMeters.createHistogram('ai_tool_search_policy_store_duration_ms', {
    description: 'Duration of policy store search operations',
    unit: 'ms',
});
const searchPolicyStoreResultsHistogram = appMeters.createHistogram('ai_tool_search_policy_store_results_count', {
    description: 'Number of results returned by policy store search',
    unit: '1',
});
const searchPolicyStoreErrorCounter = appMeters.createCounter('ai_tool_search_policy_store_errors_total', {
    description: 'Total number of policy store search errors',
    unit: '1',
});
export const searchPolicyStore = async ({ query, options, }) => {
    ...
    try {
        ...
        log((l) => l.trace('searchPolicyStore invoked.', {
            query,
            options,
            resultCount,
            durationMs: duration,
...
export const searchPolicyStoreConfig = {
    ...
    inputSchema: {
        ...
        options: PolicySearchOptionsSchema.optional().describe('Options used to influence the search results, such as scope and pagination.'),
    },
...
//# sourceMappingURL=searchPolicyStore.js.map