import { LoggedError, isError, log } from '@compliance-theater/logger';
import { SEQUENTIAL_THINKING_TOOL, SequentialThinkingServer, } from './sequential-thinking-server';
import z from 'zod';
let sequentialThinkingTool = undefined;
export const sequentialThinkingCallback = (arg) => {
    try {
        if (!sequentialThinkingTool) {
            sequentialThinkingTool = new SequentialThinkingServer();
        }
        let ret = sequentialThinkingTool.processThought(arg);
        if ('isError' in ret && ret.isError === true) {
            ...
            log((l) => l.warn('Sequential thinking tool invocation error', {
                input: arg,
                result: ret,
            }));
        }
