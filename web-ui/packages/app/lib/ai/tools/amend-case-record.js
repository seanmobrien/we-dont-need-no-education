...
import { callToActionDetails, callToActionDetailsCallToActionResponse, callToActionResponseDetails, documentProperty, documentUnits, keyPointsDetails, violationDetails, } from '@compliance-theater/database/schema';
...
import { drizDb, addDocumentRelations, addNotesToDocument, } from '@compliance-theater/database/orm';
...
const amendCaseRecordCounter = appMeters.createCounter('ai_tool_amend_case_record_total', {
    description: 'Total number of case record amendment operations',
    unit: '1',
});
const amendCaseRecordDurationHistogram = appMeters.createHistogram('ai_tool_amend_case_record_duration_ms', {
    description: 'Duration of case record amendment operations',
    unit: 'ms',
});
const amendmentRecordsHistogram = appMeters.createHistogram('ai_tool_amendment_records_count', {
    description: 'Number of records updated/inserted per amendment operation',
    unit: '1',
});
const amendmentErrorCounter = appMeters.createCounter('ai_tool_amend_case_record_errors_total', {
    description: 'Total number of case record amendment errors',
    unit: '1',
});
const updateMainRecord = async (tx, du, { severityRating, severityReasons, complianceRating, complianceReasons, completionRating, completionReasons, sentimentRating, sentimentReasons, chapter13Rating, chapter13Reasons, titleIXRating, titleIXReasons, updated, failed, }) => {
    if (!(severityRating ??
    ...
    switch (du.documentType) {
        ...
        case 'cta_response':
            await tx
                ...
                .where(eq(callToActionResponseDetails.propertyId, du.documentPropertyId))
                .execute();
            break;
        case 'key_point':
            await tx
        ...
        case 'compliance':
            ...
            break;
        default:
            log((l) => l.warn('Attempted to update main record for unsupported document type', {
...
const addNotes = async ({ tx, notes: notesFromProps, target: { unitId }, insertedRecords, failedRecords, }) => {
    if (notesFromProps?.length) {
        try {
            ...
            insertedRecords.push(...notes.map((note) => ({
                id: note.propertyId,
            ...
            failedRecords.push(...notesFromProps.map((note) => ({
                id: note,
...
export const addViolations = async ({ tx, violations, insertedRecords, failedRecords, target: { unitId: emailDocumentId, emailId }, }) => {
    if (violations?.length) {
        const violationRecords = violations.map(({ violationType, severityLevel, severityReasons, violationReasons, titleIxRelevancy, chapt13Relevancy, ferpaRelevancy, otherRelevancy, }) => {
            return {
                propertyId: newUuid(),
                emailDocumentId,
                violationType,
        ...
        try {
            ...
            await tx
                ...
                .values(violationRecords.map(({ violationType, propertyId }) => ({
                emailId,
            ...
            insertedRecords.push(...violationRecords.map((record) => ({
            ...
            failedRecords.push(...violationRecords.map((record) => ({
                id: record.propertyId,
...
const associateResponsiveActions = async ({ tx, associateResponsiveAction, target: { documentType, documentPropertyId }, }) => {
    if (!associateResponsiveAction?.length) {
        return;
    }
    if (documentType !== 'cta_response') {
        throw new Error('The source document id must be a CTA Response to associate with a CTA.');
    }
    if (!documentPropertyId) {
        throw new Error('The source document id must have a document property ID to associate with a CTA.');
    }
    const targetActions = (await tx.query.documentUnits.findMany({
        where: (documentUnits, { inArray, eq, and }) => and(inArray(documentUnits.unitId, associateResponsiveAction.map((m) => m.relatedCtaDocumentId)), eq(documentUnits.documentType, 'cta')),
        with: {
            docProp: {
                columns: {},
                with: {
    ...
    })).filter((v) => (v.docProp?.cta?.responses?.map((r) => r.ctaResponse?.propertyId ?? 0) ??
        []).includes(documentPropertyId));
    if (!targetActions || targetActions.length === 0) {
        throw new Error('All target documents must refer to a call to action record.');
    }
    const records = associateResponsiveAction
        .map((ra) => {
        const cta = targetActions.find((ta) => ta.unitId === ra.relatedCtaDocumentId);
        if (!cta) {
            return undefined;
        }
        return {
...
const relateDocuments = async ({ tx, target: { unitId }, insertedRecords, addRelatedDocuments, }) => {
    if (!addRelatedDocuments || !addRelatedDocuments.length) {
        return;
    }
    const records = await addDocumentRelations({
        db: tx,
        addDocumentRelations: addRelatedDocuments.map(({ relatedToDocumentId, relationshipType }) => ({
            sourceDocumentId: unitId,
            targetDocumentId: relatedToDocumentId,
            relationshipReasonId: relationshipType,
        })),
    });
    records.forEach(({ targetDocumentId, sourceDocumentId, relationshipReasonId }) => {
...
export const amendCaseRecord = async ({ update: { targetCaseFileId, notes, violations, explanation, addRelatedDocuments, associateResponsiveAction, ...props }, }) => {
    ...
    if (!explanation || explanation.trim().length === 0) {
        ...
        return toolCallbackResultFactory(new Error('Explanation is required'));
    }
    try {
        ...
        if (!targetDocumentId) {
            ...
            return toolCallbackResultFactory(new Error('Target case file ID not found'));
        }
        const target = await drizDb().query.documentUnits.findFirst({
            where: (documentUnits, { eq }) => eq(documentUnits.unitId, targetDocumentId),
            columns: {
                content: false,
            },
            with: {
...
export const amendCaseRecordConfig = {
    description: 'This tool supports updating values within existing case file documents.  It provides the following capabilities:\n' +
...
//# sourceMappingURL=amend-case-record.js.map