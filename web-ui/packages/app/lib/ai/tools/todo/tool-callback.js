...
const DEFAULT_LIST_PRIORITY = 'medium';
...
function serializeTodoList(list) {
    return {
        id: list.id,
        title: list.title,
...
export const createTodoCallback = async ({ listId, title, description, status, priority, todos, }) => {
    try {
        const session = await auth();
        const userId = session?.user?.id;
        log((l) => l.info('createTodoList tool called', {
            listId,
            title,
...
`,
    inputSchema: {
        listId: z
            .string()
            .optional()
            .describe('Optional identifier for the list. When provided, the existing list with this id is replaced.'),
        title: z.string().describe('Title for the todo list.'),
        description: z
            .string()
            .optional()
            .describe('Optional narrative explaining the list purpose or case context.'),
        status: z
        ...
        todos: z
            ...
            id: z
                .string()
                .optional()
                .describe('Optional stable identifier for the todo. Provide when replacing lists to preserve references.'),
            ...
            .optional()
            .describe('Complete set of todos for the list. Items not included here will be removed when replacing an existing list.'),
    },
...
export const getTodosCallback = async ({ completed, listId, }) => {
    try {
        const session = await auth();
        const userId = session?.user?.id;
        ...
        return toolCallbackResultFactory(lists.map((list) => serializeTodoList(list)));
    }
    catch (error) {
        LoggedError.isTurtlesAllTheWayDownBaby(error, {
            source: 'getTodos',
...
`,
    inputSchema: {
        completed: z
            .boolean()
            .optional()
            .describe('Optional filter - true for completed todos, false for incomplete todos, omit for all todos'),
        listId: z
            .string()
            .optional()
            .describe('Optional list identifier. When provided, returns only the matching todo list.'),
    },
    outputSchema: {
        result: z.union([
            z.object({
...
export const updateTodoCallback = async ({ id, title, description, completed, status, priority, }) => {
    try {
        const session = await auth();
        const userId = session?.user?.id;
        log((l) => l.info(`update todo called`, {
            id,
            title,
            description,
            completed,
        ...
        if (!todo) {
            return toolCallbackResultFactory(new Error(`Todo with id ${id} not found`), `Todo with id ${id} not found`);
        }
...
export const deleteTodoCallback = async ({ id }) => {
    try {
        const session = await auth();
        const userId = session?.user?.id;
...
export const toggleTodoCallback = async ({ id }) => {
    try {
        const session = await auth();
        const userId = session?.user?.id;
...
export const toggleTodoConfig = {
    description: 'Advance a todo through pending → active → complete and automatically update the parent list status (active when work starts, complete when all tasks finish).',
    inputSchema: {
...
//# sourceMappingURL=tool-callback.js.map