...
import { createInstrumentedSpan, reportEvent, } from '@/lib/nextjs-util/server/utils';
...
export default class MemoryClient {
    ...
    _validateOrgProject() {
        if ((!this.organizationName && !!this.projectName) ||
            ...
            log((l) => l.silly('Warning: Both organizationName and projectName must be provided together when using either. This will be removed from version 1.0.40. Note that organizationName/projectName are being deprecated in favor of organizationId/projectId.'));
        }
    }
    constructor(options) {
    ...
    async #updateAuthorizationIfNeeded() {
        ...
        if (thisImpersonate) {
            try {
                ...
                if (impersonatedToken) {
                    ...
                    log((l) => l.verbose(`mem0 client is impersonating ${thisImpersonate.getUserContext().userId}.`));
                }
                else {
                    log((l) => l.warn(`mem0 client unable to impersonate user, request will be made anonymously or via API key`));
                }
            }
            catch (error) {
                LoggedError.isTurtlesAllTheWayDownBaby(error, {
    ...
    #resolveForwardedApiUrl(url) {
        const joinPathSegments = (base, target) => {
        ...
        const alreadyPrefixed = sanitizedBase.length > 0 &&
            (trimmedRelative === sanitizedBase ||
                trimmedRelative.startsWith(`${sanitizedBase}/`));
        const shouldBypassBase = isSwaggerDocs ||
    ...
    async _instrument(name, fn) {
        ...
        const attributes = {
            ...
            'mem0.org.id': this.organizationId !== null && this.organizationId !== undefined
                ? String(this.organizationId)
                : '',
            'mem0.project.id': this.projectId !== null && this.projectId !== undefined
    ...
    async ping() {
        return this._instrument('ping', async () => {
            try {
                const response = await this._fetchWithErrorHandling('ping/', {
                    method: 'GET',
                });
                ...
                if (error instanceof APIError) {
                    ...
                    throw new APIError(`Failed to ping server: ${error.message || 'Unknown error'}`);
                }
            }
        });
    }
    ...
    async update(memoryId, message) {
        return this._instrument('update', async (span) => {
            ...
            const response = await this._fetchWithErrorHandling(`memories/${memoryId}/`, {
                method: 'PUT',
                headers: this.headers,
                body: JSON.stringify(payload),
            });
    ...
    async getAll(options) {
        return this._instrument('getAll', async (span) => {
            ...
            if (this.organizationId != null && this.projectId != null) {
                ...
                if (otherOptions.org_name)
                    delete otherOptions.org_name;
                if (otherOptions.project_name)
                    delete otherOptions.project_name;
            }
            if (api_version === 'v2') {
    ...
    async search(query, options) {
        return this._instrument('search', async (span) => {
            ...
            const endpoint = api_version === 'v2' ? 'v2/memories/search/' : 'memories/search/';
    ...
    async deleteAll(options = {}) {
        return this._instrument('deleteAll', async (span) => {
            ...
            const response = await this._fetchWithErrorHandling(`memories/?${params}`, {
                method: 'DELETE',
                headers: this.headers,
            });
            return response;
    ...
    async history(memoryId) {
        return this._instrument('history', async (span) => {
            ...
            const response = await this._fetchWithErrorHandling(`memories/${memoryId}/history/`, {
                headers: this.headers,
            });
            return response;
        });
    ...
    async users() {
        return this._instrument('users', async (span) => {
            ...
            const response = await this._fetchWithErrorHandling(`entities/?${params}`, {
                headers: this.headers,
            });
            return response;
        });
    ...
    async deleteUser(data) {
        ...
        const response = await this._fetchWithErrorHandling(`entities/${data.entity_type}/${data.entity_id}/`, {
            method: 'DELETE',
            headers: this.headers,
        });
        return response;
    ...
    async deleteUsers(params = {}) {
        return this._instrument('deleteUsers', async (span) => {
            ...
            for (const entity of to_delete) {
                try {
                    await this._fetchWithErrorHandling(`v2/entities/${entity.type}/${entity.name}/${qp.size > 0 ? `?${qp.toString()}` : ''}`, {
                        method: 'DELETE',
                    });
                }
                catch (error) {
                    throw new APIError(`Failed to delete ${entity.type} ${entity.name}: ${error.message}`);
                }
            }
    ...
    async getProject(options) {
        return this._instrument('getProject', async (span) => {
            ...
            const response = await this._fetchWithErrorHandling(`orgs/organizations/${this.organizationId}/projects/${this.projectId}/?${params.toString()}`, {
                headers: this.headers,
            });
            return response;
        });
    ...
    async updateProject(prompts) {
        return this._instrument('updateProject', async (span) => {
            ...
            const response = await this._fetchWithErrorHandling(`orgs/organizations/${this.organizationId}/projects/${this.projectId}/`, {
                method: 'PATCH',
                headers: this.headers,
    ...
    async getWebhooks(data) {
        return this._instrument('getWebhooks', async (span) => {
            ...
            const response = await this._fetchWithErrorHandling(`webhooks/projects/${project_id}/`, {
                headers: this.headers,
            });
            return response;
        });
    ...
    async createWebhook(webhook) {
        return this._instrument('createWebhook', async (span) => {
            ...
            const response = await this._fetchWithErrorHandling(`webhooks/projects/${this.projectId}/`, {
                method: 'POST',
                headers: this.headers,
    ...
    async updateWebhook(webhook) {
        return this._instrument('updateWebhook', async (span) => {
            ...
            const response = await this._fetchWithErrorHandling(`webhooks/${webhook.webhookId}/`, {
                method: 'PUT',
                headers: this.headers,
                body: JSON.stringify({
                    ...webhook,
    ...
    async deleteWebhook(data) {
        return this._instrument('deleteWebhook', async (span) => {
            ...
            const response = await this._fetchWithErrorHandling(`webhooks/${webhook_id}/`, {
                method: 'DELETE',
                headers: this.headers,
            });
            return response;
