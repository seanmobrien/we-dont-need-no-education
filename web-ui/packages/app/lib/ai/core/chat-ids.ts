...
export const generateChatId = (
  ...
  let id = '';
  if (!seed) {
    // Use crypto-secure random bytes for seedless chat ID generation
    // Use rejection sampling to avoid modulo bias
    let needed = 8;
    let collected = 0;
    while (collected < needed) {
      const bytes = crypto.randomBytes(needed - collected);
      for (let i = 0; i < bytes.length && collected < needed; i++) {
        // 256 % chars.length = 16, so only use bytes < 240
        ...
        const cb = chars[idx];
        if (cb === undefined) {
          log((l) => l.error('Chat ID generation failed (crypto)', { idx, byte: bytes[i] }));
        }
        id += cb ?? '';
        collected++;
      }
    }
    // Non-deterministic: use generated bytes as seed source for reference
    actualSeed = Number.fromBytes ? Number.fromBytes(bytes) : bytes.readUIntBE(0, Math.min(6, bytes.length));
