...
export class HybridSearchClient {
    ...
    static parseResponse = (json, query, options) => {
        if (json.error) {
            const { code, message } = json.error;
            throw new Error(`Error in search response: ${message || '[no message]'}` +
        ...
        return {
            results: json.value
                ...
                try {
                    return {
                        id: HybridSearchClient.parseId(doc.metadata) ??
                            doc.id,
                        content: doc.content,
                        metadata: HybridSearchClient.parseMetadata(doc.metadata) ??
                            doc.metadata,
    ...
    embeddingService;
    constructor(embeddingServiceOrOptions) {
        if (!embeddingServiceOrOptions) {
            ...
            if ('embeddingService' in embeddingServiceOrOptions) {
                ...
                throw new Error('Invalid argument: expected an IEmbeddingService or an object with embeddingService property', { cause: embeddingServiceOrOptions });
            }
        }
    }
    getSearchApiVersion() {
    ...
    getServiceUrl() {
        return (`${env('AZURE_AISEARCH_ENDPOINT')}` +
            `/indexes/${this.getSearchIndexName()}` +
            `/docs/search?api-version=${this.getSearchApiVersion()}`);
    }
    async hybridSearch(naturalQuery, options) {
        const { hitsPerPage: topK = 5, page = 1, exhaustive = false, } = options ?? {};
        ...
        try {
            ...
            const ret = HybridSearchClient.parseResponse(body, naturalQuery, options ?? {});
...
//# sourceMappingURL=HybridSearchBase.js.map