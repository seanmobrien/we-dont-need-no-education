...
import { globalRequiredSingleton, isNotNull, SingletonProvider, } from '@compliance-theater/typescript';
import { isAutoRefreshFeatureFlag } from '@compliance-theater/feature-flags/feature-flag-with-refresh';
import { isPromise } from 'util/types';
import { SupportedProviders, normalizeModelKeyForProvider, getModelFlag, caseProviderMatch, } from './util';
import { getAvailability } from './model-availability-manager';
const setupMiddleware = async (provider, model) => {
...
const getGuardedProvider = async (provider, cb) => {
...
const customProviderFactory = async ({ provider, providerFactory, optionsFactory: optionsFactoryFromProps, baselineFactory: baselineFactoryFromProps, apiKey, }) => {
    const optionsFactory = optionsFactoryFromProps ??
        ((cfg) => ({
            baseURL: cfg.base,
            apiKey: cfg.apiKey ?? apiKey,
        }));
    const baselineFactory = baselineFactoryFromProps ?? (() => ({}));
    const modelFactory = (config, model) => {
        const merged = {
            ...(apiKey ? { apiKey } : {}),
        ...
        const builder = providerFactory(optionsFactory(merged));
        if (model === undefined) {
            return builder;
        }
        if (model === 'embedding') {
            return builder.textEmbeddingModel(merged.model);
        }
    ...
    return getGuardedProvider(provider, async (cfg) => {
        ...
        const languageModelEntries = await Promise.all(Object.keys(cfg.named ?? {}).map(async (key) => {
        ...
        const languageModels = Object.fromEntries(languageModelEntries);
...
const getAzureProvider = async () => {
    return customProviderFactory({
        ...
        baselineFactory: (model) => model === 'embedding'
            ? {
                model,
...
export const getProviderRegistry = async () => {
    ...
    return globalRequiredSingleton(GLOBAL_PROVIDER_REGISTRY, async () => {
        ...
        await Promise.all(SupportedProviders.map(async (provider) => {
            const flag = await getModelFlag(provider);
            const setupChangeEvent = (f) => {
                const cleanup = () => {
                    log((l) => l.info(`Provider ${provider} changed, refreshing global registry`));
                    Promise.allSettled(Array.from(eventHandlers.values()).map(async ([relatedFlag, relatedHandler]) => {
        ...
        const providerRegistry = createProviderRegistry(providers, {
            languageModelMiddleware: MiddlewareStateManager.Instance.getMiddlewareInstance(),
        });
        log((l) => l.info(`=== A new provider registry has been created ===`));
        return providerRegistry;
    });
};
export const aiModelFactory = async (modelType) => {
            ...
            case caseProviderMatch('azure:', modelType): {
                if (getAvailability().isModelAvailable(azureModelKey)) {
                    try {
                        ...
                        getAvailability().temporarilyDisableModel(azureModelKey, 60000);
                        log((l) => l.warn(`Azure model ${modelType} failed, temporarily disabled:`, error));
                    }
                }
    ...
    switch (modelType) {
        ...
        case caseProviderMatch('google:', modelType):
            const googleEmbed = (await getProviderRegistry()).textEmbeddingModel(googleModelKey);
            if (googleEmbed != null) {
                return googleEmbed;
            }
            break;
        case caseProviderMatch('openai:', modelType):
            const openaiEmbed = (await getProviderRegistry()).textEmbeddingModel(openaiModelKey);
            if (openaiEmbed != null) {
                return openaiEmbed;
            }
            break;
    ...
    throw new TypeError(`Invalid AiModelType provided (${modelType}).  Expected one of the aliased names: $'hifi', 'lofi', \
      'completions']} or a provider-prefixed model name like 'azure:chatgtp-4o-minni', 'google:gemini-flash-2.0', or 'openai:gpt-4'.`, {
        cause: modelType,
    });
};
export const createEmbeddingModel = async () => aiModelFactory(AiModelTypeValue_Embedding);
export const createGoogleEmbeddingModel = async () => aiModelFactory(AiModelTypeValue_GoogleEmbedding);
