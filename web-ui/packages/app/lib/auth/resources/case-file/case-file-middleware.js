...
import { checkCaseFileAccess } from './case-file-resource';
...
export const checkCaseFileAuthorization = async (req, caseFileDocumentId, options) => {
    try {
        ...
        if (!userId) {
            ...
            return {
                authorized: false,
                response: NextResponse.json({ error: 'Case file not found for this document unit' }, { status: 404 }),
            };
        }
        ...
        const hasAccess = await checkCaseFileAccess(req, userId, options.requiredScope);
        if (!hasAccess) {
            log((l) => l.warn({
...
//# sourceMappingURL=case-file-middleware.js.map