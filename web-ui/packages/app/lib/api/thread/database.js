...
export class ThreadRepository {
    ...
    async list(pagination) {
        ...
        try {
            const results = await query((sql) => sql `SELECT * FROM threads ORDER BY created_date DESC LIMIT ${num} OFFSET ${offset}`, { transform: ThreadRepository.MapRecordToSummary });
            if (results.length === page) {
                const total = await query((sql) => sql `SELECT COUNT(*) as records FROM threads`);
                return {
                    results,
                    pageStats: {
                        num,
    ...
    async get(threadId) {
        try {
            ...
            if (typeof threadId === 'string') {
                result = await query((sql) => sql `SELECT * FROM threads WHERE external_id = ${threadId}`, { transform: ThreadRepository.MapRecordToSummary });
            }
            else {
                result = await query((sql) => sql `SELECT * FROM threads WHERE thread_id = ${threadId}`, { transform: ThreadRepository.MapRecordToSummary });
            }
    ...
    async create({ subject, externalId, createdOn, }) {
        try {
            ...
            const result = await drizDbWithInit(async (db) => await db
    ...
    async update({ threadId, subject, externalId, }) {
        if (!threadId) {
            throw new ValidationError({
                field: 'threadId',
                source: 'ThreadRepository',
        ...
        try {
            const result = await queryExt((sql) => sql(`UPDATE threads SET ${updateFields.join(', ')} WHERE thread_id = $${paramIndex} RETURNING *`.toString(), values), { transform: ThreadRepository.MapRecordToSummary });
