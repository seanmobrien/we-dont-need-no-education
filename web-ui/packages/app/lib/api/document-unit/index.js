...
export class DocumentUnitRepository extends BaseObjectRepository {
    ...
    #pendingEmbed = false;
    constructor({ generateDownloadKey = false, alwaysReturnContent = false, pendingEmbed = false, } = {}) {
        super({
            tableName: 'document_units',
            idField: ['unitId', 'unit_id'],
            objectMap: 'mapToDocumentUnit',
            summaryMap: alwaysReturnContent ?? false
                ? 'mapToDocumentUnit'
  ...
  (
    ...
    async getQueryProperties(recordId) {
  ...
  (
    ...
    getCreateQueryProperties({ emailId, attachmentId, emailPropertyId, content, documentType, embeddingModel, userId, }) {
        return [
    ...
    getUpdateQueryProperties({ content, documentType, embeddingModel, embeddedOn, }) {
        return [
            {
                content,
    ...
    mapToDocumentUnitSummary = (record) => {
        const ret = {
