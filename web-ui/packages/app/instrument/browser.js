...
import { addSendCustomEventListener, } from '@compliance-theater/logger';
...
let unsubscribeSendCustomEvent;
const ensureSendCustomEventListener = () => {
    if (unsubscribeSendCustomEvent)
        return;
    const handler = (payload) => {
        ...
        if (appInsights?.trackEvent) {
            appInsights.trackEvent({ name: payload.event.event }, payload.event.measurements);
...
const getAppInsights = () => {
    if (env('NEXT_PUBLIC_AZURE_MONITOR_CONNECTION_STRING') &&
        ...
        if (!appInsightState.initializersAdded) {
            ...
            appInsightState.appInsightInstance.addTelemetryInitializer((envelope) => {
                ...
                envelope.tags['ai.cloud.roleInstance'] = `${config.attributes['service.instance'] ?? 'service-instance'}-browser`.replace('WebUi--undefined', 'ObApps.ComplianceTheatre-WebUi');
                envelope.data ??= {};
