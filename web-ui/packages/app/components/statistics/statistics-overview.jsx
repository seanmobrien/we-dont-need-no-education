...
export const StatisticsOverview = () => {
    ...
    const { models, queues, isLoading, isError, error, refetch } = useStatistics(dataSource);
    if (isLoading) {
    ...
    if (isError) {
        return (<Alert severity="error" action={<IconButton onClick={refetch} color="inherit" size="small">
            <RefreshIcon />
          </IconButton>}>
    ...
    return (<Box>
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
        ...
        <Box display="flex" alignItems="center" gap={2}>
          <FormControlLabel control={<Switch checked={dataSource === 'redis'} onChange={(e) => setDataSource(e.target.checked ? 'redis' : 'database')}/>} label={<Typography variant="body2" color="var(--color-secondary-accent)">
                Redis Data
              </Typography>}/>
        ...
        <Grid size={{ xs: 12, sm: 6, md: 3 }}>
          <Card className="secondary">
            <CardContent>
              ...
              <Chip size="small" icon={<CheckCircleIcon />} label={`${Math.round((activeModels / totalModels) * 100)}%`} color="success"/>
            </CardContent>
          </Card>
        </Grid>
        <Grid size={{ xs: 12, sm: 6, md: 3 }}>
          <Card className="accent">
            <CardContent>
              ...
              <Chip size="small" icon={availableModels === activeModels ? (<CheckCircleIcon />) : (<ErrorIcon />)} label={`${Math.round((availableModels / activeModels) * 100)}%`} color={availableModels === activeModels ? 'success' : 'warning'}/>
            </CardContent>
          </Card>
        </Grid>
        <Grid size={{ xs: 12, sm: 6, md: 3 }}>
          <Card className="primary">
            <CardContent>
              ...
              <Typography variant="h4" color={queues.data.summary.totalPending > 0
            ? 'warning.main'
            : 'text.primary'}>
                {queues.data.summary.totalPending}
              </Typography>
              {queues.data.summary.totalPending > 0 && (<Chip size="small" icon={<ErrorIcon />} label="Pending" color="warning"/>)}
            </CardContent>
          </Card>
        </Grid>
      </Grid>
      ...
      <Accordion>
        <AccordionSummary expandIcon={<ExpandMoreIcon />}>
          <Typography variant="h6" color="var(--color-primary-accent)">
            Queue Details{' '}
            {queues.data.summary.totalPending > 0 && (<Chip size="small" label={queues.data.summary.totalPending} color="warning"/>)}
          </Typography>
        </AccordionSummary>
        <AccordionDetails>
...
//# sourceMappingURL=statistics-overview.jsx.map