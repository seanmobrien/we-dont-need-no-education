...
export const ModelStatistics = ({ models }) => {
    if (models.length === 0) {
            ...
            return (<TableRow key={model.id} hover>
                ...
                <TableCell>
                  <Box display="flex" gap={1} flexWrap="wrap">
                    {model.isActive ? (<Chip size="small" icon={<CheckCircleIcon />} label="Active" color="primary" className="primary"/>) : (<Chip size="small" icon={<PauseCircleIcon />} label="Inactive" color="default" className="secondary"/>)}
                    {model.isActive && (<Chip size="small" icon={model.available ? <CheckCircleIcon /> : <ErrorIcon />} label={model.available ? 'Available' : 'Unavailable'} color={model.available ? 'success' : 'error'} className={model.available ? 'accent' : 'primary'}/>)}
                  </Box>
                </TableCell>
                <TableCell align="center">
                  <Box minWidth={100}>
                    ...
                    {model.maxTokensPerMinute && (<Tooltip title={`${minuteUsage.toFixed(1)}% of limit`}>
                        <LinearProgress variant="determinate" value={minuteUsage} color={getUsageColor(minuteUsage)} sx={{ mt: 0.5, height: 4 }}/>
                      </Tooltip>)}
                  </Box>
                </TableCell>
                <TableCell align="center">
                ...
                <TableCell align="center">
                  <Box minWidth={100}>
                    ...
                    {model.maxTokensPerDay && (<Tooltip title={`${dayUsage.toFixed(1)}% of daily limit`}>
                        <LinearProgress variant="determinate" value={dayUsage} color={getUsageColor(dayUsage)} sx={{ mt: 0.5, height: 4 }}/>
                      </Tooltip>)}
                  </Box>
                </TableCell>
                <TableCell align="center">
...
//# sourceMappingURL=model-statistics.jsx.map