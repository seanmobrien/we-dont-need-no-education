...
export const EmailDashboardLayout = ({ children, session, }) => {
    ...
    const renderPageItem = useCallback((item, { mini }) => {
        const emailChildren = [
            'key-points',
            'notes',
            'call-to-action',
        ...
        if (dynamicMenus.includes(item.title ?? '')) {
            return (<CustomEmailPageItem item={item} mini={mini} emailId={emailId} data-id={`navmenu-email-${item.segment}`} pathname={pathname}/>);
        }
        return <DashboardSidebarPageItem item={item}/>;
    }, [emailId, pathname]);
    return (<EmailContextProvider>
      <ServerSafeErrorManager />
      <NextAppProvider theme={theme} navigation={dashboardNavigation} branding={Branding} session={session ?? null}>
        <DashboardLayout renderPageItem={renderPageItem} slots={stableDashboardSlots} sx={stableDashboardSx}>
...
//# sourceMappingURL=email-dashboard-layout.jsx.map