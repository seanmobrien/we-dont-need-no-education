...
import { setUuid, getUuid, setRecordDirty, isRecordDirty, } from '@compliance-theater/typescript';
...
const BulkEmailForm = () => {
    ...
    const [bulkUpdateOperation, setBulkUpdateOperation] = useState(null);
    const [pageStats, setPageStats] = useState({
        page: 1,
        num: 2,
        total: 0,
    });
    const fetchPageStats = useCallback((page) => {
        setLoading(true);
    ...
    const handleSave = () => {
        ...
        const isSameEmail = (x, y) => x.subject === y.subject &&
            x.sentOn === y.sentOn &&
        ...
        Promise.all(bulkUpdate.updates.map((update) => update.operation))
            ...
            setEmails((prevEmails) => prevEmails.map((email) => {
                const result = results.find((res) => res &&
    ...
    const handleStringInputChange = (event) => handleFieldChange(event, (target, field, msg) => {
    ...
    const handleDateInputChange = (event) => handleFieldChange(event, (target, field, msg) => {
        if (field === 'sentOn') {
    ...
    return (<div style={stableStyles.container}>
      ...
      {loading ? (<p style={stableStyles.loadingText}>Loading emails...</p>) : bulkUpdateOperation ? (<p style={stableStyles.loadingText}>Saving emails...</p>) : (<div>
          <table style={stableStyles.table}>
            ...
            <tbody>
              {emails.map((email, index) => (<React.Fragment key={email.emailId !== '' ? email.emailId : getUuid(email)}>
                  <tr data-row-index={index}>
                    <td style={stableStyles.tableCellTop}>
                      <ContactDropdown style={stableStyles.minWidthMax} displayValue="name" contact={email.sender} setValue={(value) => setEmails((prevEmails) => {
                    const updatedEmails = [...prevEmails];
                    ...
                    <td style={stableStyles.tableCellTopOnly}>
                      <ContactRecipients contacts={email.recipients} onContactsUpdate={(updatedContacts) => setEmails((prevEmails) => {
                    const updatedEmails = [...prevEmails];
                    ...
                    <td style={stableStyles.tableCellTopOnly}>
                      <input aria-label="Sent On" type="datetime-local" value={normalizeDateAndTime(email.sentOn)} data-field="sentOn" onChange={handleDateInputChange}/>
                    </td>
                    ...
                    if (emailToDelete.emailId) {
                        fetch(`/api/email/${emailToDelete.emailId}`, {
                            ...
                            .catch((error) => {
                            log((l) => l.error('Error deleting email:', error));
                  ...
                  <tr data-row-index={index}>
                    ...
                    <td colSpan={2} style={stableStyles.tableCellTopOnly}>
                      <EmailSelect selectedEmail={email.parentEmailId} onEmailSelect={(newParentId) => {
                    setEmails((prevEmails) => {
                        const updatedEmails = [...prevEmails];
                  ...
                  <tr data-row-index={index}>
                    ...
                    <td colSpan={2} style={stableStyles.tableCellTopOnly}>
                      <input aria-label="Subject" type="text" data-field="subject" value={email.subject} onChange={handleStringInputChange}/>
                    </td>
                  </tr>
                  ...
                  <tr data-row-index={index}>
                    ...
                    <td colSpan={2} style={stableStyles.tableCellBottomSpan}>
                      <textarea aria-label="Body" style={stableStyles.textareaFull} rows={4} data-field="body" value={email.body} onChange={handleStringInputChange}/>
                    </td>
                  </tr>
                </React.Fragment>))}
            </tbody>
          ...
          <div style={stableStyles.buttonContainer}>
            <button onClick={() => handlePageChange(pageStats.page - 1)} disabled={pageStats.page === 1} style={{
                ...stableStyles.button,
            ...
            <button onClick={() => handlePageChange(pageStats.page + 1)} disabled={pageStats.page * pageStats.num >= pageStats.total} style={{
                ...stableStyles.button,
                ...stableStyles.buttonBlue,
            }}>
              Next
