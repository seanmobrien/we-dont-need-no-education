...
export const FloatingTodoDialog = ({ listId, open, onClose, }) => {
    ...
    if (!open)
        return null;
    const dialogContent = (<Paper elevation={8} sx={{
            width: '100%',
            height: '100%',
            display: 'flex',
      ...
      <DialogTitle className="drag-handle" sx={{
        ...
        <IconButton size="small" onClick={onClose} aria-label="Close dialog" sx={{ ml: 1 }}>
          <CloseIcon />
        </IconButton>
      </DialogTitle>
      <Box sx={{ flex: 1, overflow: 'auto', p: 0 }}>
        ...
        {list && list.todos.length === 0 && (<Box sx={{ p: 2 }}>
            <Typography color="text.secondary">
              No items in this list
            </Typography>
          </Box>)}

        {list && list.todos.length > 0 && (<List sx={{ py: 0 }}>
            {list.todos.map((todo) => (<ListItem key={todo.id} dense sx={{
                ...
                <Checkbox edge="start" checked={todo.completed} onChange={() => handleToggle(todo)} tabIndex={-1} disableRipple inputProps={{
                    'aria-labelledby': `todo-${todo.id}-label`,
                }}/>
                <ListItemText id={`todo-${todo.id}-label`} primary={todo.title} secondary={todo.description} sx={{
    ...
    return (<ClientWrapper>
      <Draggable handle=".drag-handle" bounds="body" nodeRef={nodeRef}>
        <Box ref={nodeRef} sx={{
          ...
          <ResizableBox width={size.width} height={size.height} onResize={handleResize} minConstraints={[300, 200]} maxConstraints={[800, 800]} resizeHandles={['se']}>
            {dialogContent}
          </ResizableBox>
        </Box>
      </Draggable>
...
//# sourceMappingURL=floating-todo-dialog.jsx.map