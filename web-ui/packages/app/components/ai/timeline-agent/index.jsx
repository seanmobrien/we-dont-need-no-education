...
const getComplianceIcon = (rating) => {
    switch (rating) {
        case ComplianceRating.Excellent:
        case ComplianceRating.Good:
...
export const TimelineAgentInterface = ({ initialDocumentId, caseId, }) => {
    ...
    const { mutate, isPending, isSuccess, isError, data, ...otherMutationProps } = useMutation({
    ...
    const loadSnapshot = useCallback((file) => {
    ...
    const progressPercentage = documentCounts.total > 0
    ...
    return (<Box sx={{
      ...
      <Paper elevation={2} sx={{ p: 2 }}>
        <Box sx={{
          ...
          <Typography variant="h4" component="h1" color="var(--color-secondary-accent)">
            Timeline Analysis Agent
            {!isPending && state.isInitialized && !state.isProcessing && (<Chip label="Ready" color="success" variant="filled" sx={{ ml: 3 }}/>)}
          </Typography>
          {state.error && <Alert severity="error">{state.error}</Alert>}
          <Box sx={{ display: 'flex', gap: 1 }}>
            <Tooltip title="Save Agent State">
              <span>
                <IconButton onClick={saveSnapshot} disabled={!state.isInitialized || isPending}>
                  <Save />
                </IconButton>
              </span>
            </Tooltip>
            <Tooltip title="Load Agent State">
              <span>
                <IconButton component="label" disabled={state.isProcessing || isPending}>
                  <input type="file" accept=".json" hidden onChange={handleFileInputChange} aria-label="Load agent state file"/>
                  <Upload />
                </IconButton>
              </span>
            </Tooltip>
            <Tooltip title="Refresh Summary">
              <span>
                <IconButton onClick={refreshSummary} disabled={!state.isInitialized || isPending}>
                  <Refresh />
                </IconButton>
              </span>
            </Tooltip>
            <Tooltip title="Reset Agent">
              <span>
                <IconButton onClick={resetAgent} disabled={state.isProcessing || isPending}>
                  <RestartAlt />
                </IconButton>
              </span>
            </Tooltip>
        ...
        <Typography variant="subtitle1" gutterBottom>
          ...
          <Typography component="span" color="var(--color-secondary-accent)" paddingX={1}>
            |
          </Typography>
          <Typography component="span" color="var(--color-primary-accent)">
            Initial Document:
        ...
        <Box sx={{ mt: 2 }}>
          ...
          <LinearProgress variant={isPending ? 'indeterminate' : 'determinate'} value={isPending ? undefined : progressPercentage} sx={{ height: 8, borderRadius: 4 }}/>
          {isPending && (<Typography variant="caption" color="primary" sx={{ mt: 1, display: 'block' }}>
      ...
      {state.error && (<Paper elevation={1} sx={{ p: 2, bgcolor: 'error.light', color: 'error.contrastText' }}>
          <Typography variant="body1">Error: {state.error}</Typography>
          <Button size="small" onClick={() => setState((prev) => ({ ...prev, error: null }))} sx={{ mt: 1, color: 'inherit' }}>
            Dismiss
          </Button>
        </Paper>)}
      ...
      <Box sx={{ display: 'flex', gap: 2, flexGrow: 1 }}>
        ...
        <Box sx={{
          ...
          <Card>
            <CardContent>
              <Typography variant="h6" gutterBottom color="var(--color-highlight-dark)">
                Current Document
              </Typography>
              {state.currentDocument ? (<Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          <Card>
            <CardContent>
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                <Button variant="contained" onClick={processNextDocument} disabled={!hasMoreDocuments ||
            state.isProcessing ||
            !state.isInitialized ||
            isPending} startIcon={state.isProcessing ? (<CircularProgress size={20}/>) : (<PlayArrow />)} fullWidth>
                  {state.isProcessing
            ? 'Processing...'
            : 'Process Next Document'}
                </Button>

                <Button variant="outlined" onClick={initializeAgent} disabled={state.isProcessing || isPending} startIcon={isPending ? <CircularProgress size={20}/> : <Refresh />} fullWidth>
          ...
          <Card sx={{ flexGrow: 1 }}>
            <CardContent>
              ...
              <Box sx={{ display: 'flex', gap: 1, mb: 2 }}>
                <Chip label={`${documentCounts.pending} Pending`} color="warning" size="small"/>
                <Chip label={`${documentCounts.processed} Processed`} color="success" size="small"/>
              </Box>
              {!hasMoreDocuments && documentCounts.total > 0 && (<Paper elevation={1} sx={{
                p: 1,
        ...
        <Box sx={{ width: '67%' }}>
          <Card sx={{ height: '100%' }}>
            <CardContent sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
              ...
              {state.summary ? (<Box sx={{ flexGrow: 1, overflow: 'auto' }}>
                  
                  <Accordion defaultExpanded>
                    <AccordionSummary expandIcon={<ExpandMore />}>
                      <Typography variant="subtitle1" color="var(--color-primary-accent)">
                        Case Overview
                      </Typography>
                    </AccordionSummary>
                    <AccordionDetails>
                ...
                'N/A'}
                  ...
                  <Accordion>
                    ...
                    <AccordionDetails>
                      <List dense>
                        {Object.entries(state.summary.complianceRatings).map(([key, rating]) => (<ListItem key={key}>
                              <ListItemIcon>
                                {getComplianceIcon(rating)}
                              </ListItemIcon>
                              <ListItemText primary={key.charAt(0).toUpperCase() + key.slice(1)} secondary={rating}/>
                            </ListItem>))}
                      </List>
                    </AccordionDetails>
                  </Accordion>
                  ...
                  <Accordion>
                    ...
                    <AccordionDetails>
                      <List>
                        {state.summary.sequentialActions.map((action, index) => (<React.Fragment key={action.documentId}>
                              <ListItem>
                                <ListItemText primary={`${index + 1}. ${action.summary}`} secondary={<Box>
                        ...
                        action.verbatimStatements.length >
                            0 && (<Box sx={{ mt: 1 }}>
                                            ...
                                            {action.verbatimStatements.map((statement, idx) => (<Typography key={idx} variant="caption" display="block" sx={{
                                fontStyle: 'italic',
                                ml: 1,
                            }}>
                              ...
                              {index <
                    state.summary.sequentialActions.length - 1 && (<Divider />)}
                            </React.Fragment>))}
                      </List>
                    </AccordionDetails>
                  </Accordion>
                  ...
                  {state.summary.criticalIssues.length > 0 && (<Accordion>
                      <AccordionSummary expandIcon={<ExpandMore />}>
                      ...
                      <AccordionDetails>
                        <List>
                          {state.summary.criticalIssues.map((issue, index) => (<ListItem key={index}>
                              <ListItemIcon>
                                <Warning color="error"/>
                              </ListItemIcon>
                  ...
                  {state.lastResult && (<Accordion>
                    ...
                    state.lastResult.notes.length > 0 && (<Box sx={{ mt: 1 }}>
                              <Typography variant="body2" color="text.secondary" gutterBottom>
                                Processing Notes:
                              </Typography>
                              <List dense>
                                {state.lastResult.notes.map((note, index) => (<ListItem key={index}>
