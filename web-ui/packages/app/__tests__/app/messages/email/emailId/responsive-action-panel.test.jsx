...
describe('ResponsiveActionPanel', () => {
    it('renders responsive action details correctly', async () => {
        ...
        await waitFor(() => screen.getByText('Detailed response to the call to action'));
        expect(screen.getByText('Responsive Action (response-test-id)')).toBeInTheDocument();
    ...
    it('fetches and displays related call-to-action', async () => {
        render(<ResponsiveActionPanel row={mockResponseDetails}/>);
        await waitFor(() => {
            expect(fetch).toHaveBeenCalledWith('/api/email/test-email-id/properties/call-to-action');
        });
        await waitFor(() => {
...
//# sourceMappingURL=responsive-action-panel.test.jsx.map