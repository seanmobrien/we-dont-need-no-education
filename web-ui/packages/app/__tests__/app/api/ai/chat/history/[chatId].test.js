...
describe('/api/ai/chat/history/[chatId] route', () => {
    ...
    beforeEach(() => {
        ...
        drizDbWithInit.mockResolvedValue(mockDb);
        eq.mockReturnValue('eq-condition');
        and.mockReturnValue('and-condition');
    });
    afterEach(() => {
        mockConsole.dispose();
    });
    describe('GET', () => {
        ...
        it('should handle database connection errors', async () => {
            mockConsole.setup();
            drizDbWithInit.mockRejectedValue(new Error('Database connection failed'));
            const request = new NextRequest('http://localhost:3000/api/ai/chat/history/test-chat-id');
            const params = Promise.resolve({ chatId: 'test-chat-id' });
            const response = await GET(request, { params });
            const data = await response.json();
...
//# sourceMappingURL=%5BchatId%5D.test.js.map