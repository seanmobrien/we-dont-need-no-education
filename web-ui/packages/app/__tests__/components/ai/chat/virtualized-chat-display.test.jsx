...
jest.mock('@/components/ai/chat/chat-turn-display', () => ({
    ChatTurnDisplay: ({ turn, showTurnProperties, showMessageMetadata }) => {
        if (!turn)
            return null;
        return (<div data-testid={`turn-${turn.turnId}`} data-show-properties={showTurnProperties} data-show-metadata={showMessageMetadata}>
...
describe('VirtualizedChatDisplay', () => {
    ...
    it('should test size estimation with turn properties enabled', () => {
        const turns = [mockChatTurnWithTool];
        render(<VirtualizedChatDisplay turns={turns}/>);
        fireEvent.click(screen.getByLabelText(/Show Turn Properties/));
        const virtualizerCall = jest.requireMock('@tanstack/react-virtual')
            .useVirtualizer.mock.calls[0][0];
    ...
    it('should calculate correct size for messages with content', () => {
        ...
        const estimateSize = virtualizerCall.estimateSize;
        const estimatedSize = estimateSize(0);
        expect(estimatedSize).toBeGreaterThan(200);
    });
...
//# sourceMappingURL=virtualized-chat-display.test.jsx.map