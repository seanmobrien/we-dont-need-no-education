...
describe('ResizableDraggableDialog Background Interaction', () => {
    it('should allow background clicks when modal=false', () => {
        ...
        render(<div>
        <div data-testid="background-element" onClick={mockBackgroundClick} style={{
                position: 'absolute',
                top: 0,
                left: 0,
                width: '100%',
        ...
        <ResizableDraggableDialog isOpenState={true} title="Test Dialog" modal={false} onClose={mockOnClose} width={400} height={300}>
    ...
    it('should render correctly when modal=false', () => {
        const mockOnClose = jest.fn();
        render(<ResizableDraggableDialog isOpenState={true} title="Test Dialog" modal={false} onClose={mockOnClose} width={400} height={300}>
        <div>Dialog Content</div>
      </ResizableDraggableDialog>);
        expect(screen.getByText('Test Dialog')).toBeInTheDocument();
        expect(screen.getByText('Dialog Content')).toBeInTheDocument();
...
//# sourceMappingURL=background-interaction.test.jsx.map