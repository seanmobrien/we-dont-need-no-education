...
describe('useMousePosition', () => {
    ...
    it('tracks multiple mouse movements', () => {
        const { result } = renderHook(() => useMousePosition());
        act(() => {
            window.dispatchEvent(new MouseEvent('mousemove', { clientX: 50, clientY: 75 }));
        });
    ...
    it('handles large coordinate values', () => {
        const { result } = renderHook(() => useMousePosition());
        act(() => {
            window.dispatchEvent(new MouseEvent('mousemove', { clientX: 9999, clientY: 8888 }));
        });
        expect(result.current).toEqual({ x: 9999, y: 8888 });
    });
});
//# sourceMappingURL=use-mouse-position.test.jsx.map