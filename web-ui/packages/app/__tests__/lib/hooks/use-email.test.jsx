...
const mockedGetEmail = getEmail;
const mockedWriteEmailRecord = writeEmailRecord;
const TIMEOUT = 15000;
const ErrorTestComponent = ({ triggerError, }) => {
...
describe('useEmailQuery hooks', () => {
    describe('useEmail hook', () => {
        ...
        it('should fetch email data successfully', async () => {
            ...
            mockedGetEmail.mockReturnValue(mockPromise);
        ...
        it('should handle errors properly', async () => {
            ...
            render(<ErrorTestComponent triggerError={() => useEmail('123', { enabled: true })}/>);
            await waitFor(async () => expect(screen.getByText('Error occurred')).toBeInTheDocument(), { timeout: TIMEOUT - 1000 });
        }, TIMEOUT);
    ...
    describe('useWriteEmail hook', () => {
        ...
        it('should write email data successfully', async () => {
            ...
            const mockPromise = Promise.resolve(mockEmail);
            Object.assign(mockPromise, { cancel: jest.fn() });
            mockedWriteEmailRecord.mockReturnValue(mockPromise);
        ...
        it('should handle write errors properly', async () => {
            ...
            await act(async () => {
                if (!doMutate)
                    throw new Error('mutate function not defined');
                doMutate(emailData);
            });
            await waitFor(async () => expect(screen.getByText('Error occurred')).toBeInTheDocument(), { timeout: TIMEOUT - 1000 });
...
//# sourceMappingURL=use-email.test.jsx.map