...
jest.mock('postgres');
import { sqlNeonAdapter, isSqlNeonAdapter, unwrapAdapter, query, queryExt, queryRaw, db, Resultset, } from '@compliance-theater/database/driver';
describe('sqlNeonAdapter', () => {
    it('should call sql for template queries', () => {
        const sql = jest.fn();
        const adapter = sqlNeonAdapter(sql);
        const tpl = [
    ...
    it('should attach wrappedAdapter symbol', () => {
        const sql = jest.fn();
        const adapter = sqlNeonAdapter(sql);
        const wrapped = adapter[Object.getOwnPropertySymbols(adapter)[0]];
...
describe('db', () => {
    it('should return a Resultset with transformed records', async () => {
        const cb = jest.fn().mockReturnValue(Promise.resolve({
            statement: 'SELECT',
            command: 'SELECT',
            columns: [{ name: 'id', number: 0 }],
            map: function () {
        ...
        const transform = (r) => Object.assign([{ id: r.id * 2 }], { id: r.id * 2 });
