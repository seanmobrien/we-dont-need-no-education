...
describe('KeycloakTokenExchange', () => {
    ...
    describe('extractKeycloakToken', () => {
        ...
        it('should throw error when no JWT token found', async () => {
            ...
            await expect(tokenExchange.extractKeycloakToken(mockRequest)).rejects.toThrow(TokenExchangeError);
        });
        it('should throw error when no access_token in JWT', async () => {
            mockedGetToken.mockResolvedValue({});
            await expect(tokenExchange.extractKeycloakToken(mockRequest)).rejects.toThrow('No Keycloak access token found in JWT');
        });
    });
    describe('exchangeForGoogleTokens', () => {
        it('should successfully exchange tokens', async () => {
            ...
            expect(mockedAxios.post).toHaveBeenCalledWith('https://keycloak.example.com/realms/test/protocol/openid-connect/token', expect.stringContaining('grant_type=urn%3Aietf%3Aparams%3Aoauth%3Agrant-type%3Atoken-exchange'), expect.objectContaining({
...
//# sourceMappingURL=keycloak-token-exchange.test.js.map