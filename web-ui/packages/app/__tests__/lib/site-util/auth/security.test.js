import { isUserAuthorized, isSessionActive, } from '@/lib/site-util/auth/security';
describe('isUserAuthorized', () => {
    ...
    test('owner allowed write and read', async () => {
        await expect(isUserAuthorized({ signedInUserId: 10, ownerUserId: 10, write: true })).resolves.toBe(true);
        await expect(isUserAuthorized({ signedInUserId: 10, ownerUserId: 10 })).resolves.toBe(true);
    });
    test('non-owner read allowed, write denied', async () => {
        await expect(isUserAuthorized({ signedInUserId: 3, ownerUserId: 10 })).resolves.toBe(true);
        await expect(isUserAuthorized({ signedInUserId: 3, ownerUserId: 10, write: true })).resolves.toBe(false);
    });
});
