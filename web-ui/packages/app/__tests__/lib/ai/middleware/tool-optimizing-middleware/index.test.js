...
import { createToolOptimizingMiddleware, getToolOptimizingMiddlewareMetrics, } from '@/lib/ai/middleware/tool-optimizing-middleware';
...
describe('Tool Optimizing Middleware', () => {
    ...
    describe('Tool Scanning Functionality', () => {
        it('should scan tools when tool scanning is enabled', async () => {
            ...
            expect(mockToolMapInstance.scanForTools).toHaveBeenCalledWith(sampleTools);
        ...
        it('should handle tool scanning errors gracefully', async () => {
            ...
            mockToolMapInstance.scanForTools.mockRejectedValue(new Error('Database error'));
            const middleware = createToolOptimizingMiddleware({
                enableToolScanning: true,
            });
            const result = await middleware.transformParams({
    ...
    describe('Message Optimization Functionality', () => {
        it('should optimize messages when conditions are met', async () => {
            ...
            expect(mockOptimizeMessages).toHaveBeenCalledWith(mockParams.prompt, 'gpt-4.1', 'user-123', 'chat-456');
            expect(result.prompt).toHaveLength(8);
        });
        ...
        it('should handle message optimization errors gracefully', async () => {
            ...
            expect(result.prompt.length).toBe(sampleMessages.length + 1);
        });
        it('should extract model ID from string model parameter', async () => {
            ...
            expect(mockOptimizeMessages).toHaveBeenCalledWith(sampleMessages, 'gpt-4.1', 'user-123', undefined);
        });
        it('should handle missing model gracefully', async () => {
            ...
            expect(mockOptimizeMessages).toHaveBeenCalledWith(sampleMessages, 'gpt-4.1', undefined, undefined);
        });
    });
    ...
    describe('Combined Operations', () => {
        it('should perform both tool scanning and message optimization', async () => {
            ...
            expect(mockToolMapInstance.scanForTools).toHaveBeenCalledWith(sampleTools);
            expect(mockOptimizeMessages).toHaveBeenCalledWith(sampleMessages, 'gpt-4.1', 'user-123', 'chat-456');
            expect(result.prompt).toHaveLength(8);
            expect(result.tools).toBe(sampleTools);
        });
        it('should handle partial failures gracefully', async () => {
            hideConsoleOutput().setup();
            mockToolMapInstance.scanForTools.mockRejectedValue(new Error('Tool scan failed'));
            const middleware = createToolOptimizingMiddleware({
                enableToolScanning: true,
                enableMessageOptimization: true,
                optimizationThreshold: 10,
            ...
            expect(LoggedError.isTurtlesAllTheWayDownBaby).toHaveBeenCalledWith(expect.any(Error), expect.objectContaining({
