...
import { setupMaps, PROVIDER_ID_GOOGLE, MODEL_ID_GPT4_NO_QUOTA, } from '@/__tests__/setup/jest.mock-provider-model-maps';
...
describe('TokenStatsService', () => {
    describe('normalizeModelKey', () => {
        it('should handle provider:model format', async () => {
            ...
            const result = await tokenStatsService.getTokenStats('azure:gpt-4.1', 'ignored');
            expect(result).not.toBeNull();
            expect(mockRedisClient.get).toHaveBeenCalledWith('token_stats:b555b85f-5b2f-45d8-a317-575a3ab50ff2:gpt-4.1:minute');
        });
        it('should handle separate provider and model', async () => {
            mockRedisClient.get.mockResolvedValue(null);
            const result = await tokenStatsService.getTokenStats('google', 'gemini-2.0-flash');
            expect(result).not.toBeNull();
            expect(mockRedisClient.get).toHaveBeenCalledWith(`token_stats:${PROVIDER_ID_GOOGLE}:gemini-2.0-flash:minute`);
        });
    });
    describe('getQuota', () => {
        ...
        it('should return null when no quota is configured', async () => {
            mockRedisClient.get.mockResolvedValue(null);
            const result = await tokenStatsService.getQuota('azure', 'gpt-4.1-no-quota');
            expect(result).not.toBeNull();
            expect(result?.maxTokensPerDay).toBeUndefined();
    ...
    describe('checkQuota', () => {
        it('should allow request when no quota is configured', async () => {
            mockRedisClient.get.mockResolvedValue(null);
            const result = await tokenStatsService.checkQuota('azure', 'gpt-4.1-no-quota', 100);
            expect(result.allowed).toBe(true);
        });
        it('should block request that exceeds per-message limit', async () => {
            await ModelMap.getInstance().then((x) => x.addQuotaToModel({
                modelId: MODEL_ID_GPT4_NO_QUOTA,
                maxTokensPerMessage: 500,
                isActive: true,
            }));
            const result = await tokenStatsService.checkQuota('azure', 'gpt-4.1-no-quota', 1000);
        ...
        it('should block request that exceeds per-minute limit', async () => {
            ...
            const result = await tokenStatsService.checkQuota('azure', 'gpt-4.1-no-quota', 200);
            expect(result.allowed).toBe(false);
        ...
        it('should allow request within all limits', async () => {
            ...
            const result = await tokenStatsService.checkQuota('azure', 'gpt-4.1', 400);
            expect(result.allowed).toBe(true);
        });
    });
    describe('safeRecordTokenUsage', () => {
        ...
        it('should handle Redis and database errors gracefully', async () => {
            ...
            await expect(tokenStatsService.safeRecordTokenUsage('azure', 'gpt-4.1', usage)).resolves.not.toThrow();
        });
    });
...
//# sourceMappingURL=tokenStatsService.test.js.map