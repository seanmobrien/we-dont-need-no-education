...
jest.mock('@/lib/nextjs-util/server/utils', () => ({
    createInstrumentedSpan: jest.fn(async () => ({
        executeWithContext: async (fn) => fn({}),
    })),
...
const { fromRequest: mockFromRequest, ImpersonationService: MockImpersonationService, } = jest.requireMock('@/lib/auth/impersonation');
...
const createTextResponse = (payload) => ({
    ok: true,
...
describe('MemoryClient configurable base path', () => {
    ...
    const queuePingAndReturn = (response) => {
    ...
    it('prefixes the configured base path for relative endpoints', async () => {
        ...
        expect((options?.headers)['X-Mem0-User-ID']).toBeTruthy();
    });
    ...
    it('applies the base path when invoking memoryClientFactory healthCheck', async () => {
        ...
        expect(fetchMock).toHaveBeenCalledTimes(2);
        const [url] = fetchMock.mock.calls[1];
        expect(String(url)).toEqual('https://mem0.test/api/v1/stats/health-check?strict=false&verbose=1');
    });
});
//# sourceMappingURL=memory-client.test.js.map