...
function mockGetCachedToolsProxy(..._args: unknown[]) {
...
function createMcpCacheMock() {
  return {
    getToolCache: (...args: unknown[]) => mockGetToolCache(...args),
  };
}
...
jest.mock('@ai-sdk/mcp', () => {
  ...
  return {
    ...original,
    experimental_createMCPClient: (...args: unknown[]) => mockCreateMCPClient(...args),
  };
});

function createInstrumentedSseTransportMock() {
  return {
    InstrumentedSseTransport: function (...args: unknown[]) {
...
describe('toolProviderFactory tests', () => {
