...
const mockGetResolvedPromises = jest.fn();
const mockIsError = jest.fn();
const mockCreateMCPClient = jest.fn();
const mockInstrumentedSseTransport = jest.fn();
...
jest.mock('@/lib/ai/mcp/providers/client-tool-provider', () => ({
    clientToolProviderFactory: jest.fn(() => {
        ...
        return {
            ...
            addDisposeListener: jest.fn((cb) => {
                emitter.on('dispose', cb);
            }),
            removeDisposeListener: jest.fn((cb) => {
                emitter.off('dispose', cb);
            }),
        };
    })
}));
import { toolProviderFactory, toolProviderSetFactory, } from '@/lib/ai/mcp/providers';
...
const makeDisposeEmitter = (input) => {
    const emitter = new EventEmitter();
    ...
    ret.dispose = dispose;
    ret.addDisposeListener = jest.fn((listener) => {
        emitter.on('dispose', listener);
    });
    ret.removeDisposeListener = jest.fn((listener) => {
...
describe('toolProviderFactory', () => {
    ...
    beforeEach(() => {
        ...
        mockIsError.mockImplementation((error) => error instanceof Error);
    ...
    describe('successful connection', () => {
        ...
        it('should create InstrumentedSseTransport with correct options', async () => {
            ...
            expect(mockInstrumentedSseTransport).toHaveBeenCalledWith(expect.objectContaining({
                type: 'sse',
                url: mockOptions.url,
        ...
        it('should create MCP client with transport and error handler', async () => {
            ...
            createAutoRefreshFeatureFlag.mockImplementation(async (ops) => {
                switch (ops.key) {
        ...
        it('should handle MCP client uncaught errors', async () => {
            let uncaughtErrorHandler;
            mockCreateMCPClient.mockImplementation((options) => {
        ...
        describe('provider methods', () => {
            ...
            it('should handle dispose errors gracefully', async () => {
                ...
                await provider[Symbol.dispose]();
                expect(mockLoggedError.isTurtlesAllTheWayDownBaby).toHaveBeenCalledWith(disposeError, expect.objectContaining({
                    log: true,
...
describe('toolProviderSetFactory', () => {
    ...
    const createMockProvider = (tools = mockTools) => {
        const mcpClient = {
            tools: jest.fn(() => Promise.resolve(tools)),
            close: jest.fn(),
        };
        mockCreateMCPClient.mockResolvedValueOnce(mcpClient);
        const emitter = new EventEmitter();
        const ret = {
            get_mcpClient: jest.fn(() => mcpClient),
            get_isConnected: jest.fn(() => true),
            get tools() {
                return tools;
            },
            [Symbol.dispose]: jest.fn(() => emitter.emit('dispose')),
            addDisposeListener: jest.fn((cb) => emitter.on('dispose', cb)),
            removeDisposeListener: jest.fn((cb) => emitter.off('dispose', cb)),
            connect: jest.fn(() => Promise.resolve(ret)),
        };
        return ret;
    };
    beforeEach(() => {
        createAutoRefreshFeatureFlag.mockResolvedValue({
    ...
    describe('successful connections', () => {
        ...
        it('should create provider set with all successful connections', async () => {
            ...
            expect(mockGetResolvedPromises).toHaveBeenCalledWith(expect.arrayContaining([
                expect.any(Promise),
                expect.any(Promise),
                expect.any(Promise),
            ]), expect.any(Number));
        ...
        it('should aggregate tools from all providers', async () => {
            ...
            mockGetResolvedPromises.mockImplementation(async (promises) => {
        ...
        it('should use custom timeout', async () => {
            ...
            expect(mockGetResolvedPromises).toHaveBeenCalledWith(expect.any(Array), customTimeout);
        });
    ...
    describe('partial failures', () => {
        ...
        it('should handle mixed success/failure scenarios', async () => {
            ...
            expect(mockLoggedError.isTurtlesAllTheWayDownBaby).toHaveBeenCalledWith(expect.any(AggregateError), expect.objectContaining({
                log: true,
    ...
    describe('cleanup handling', () => {
        ...
        it('should handle cleanup errors gracefully', async () => {
            ...
            const mockedDispose = failingProvider[Symbol.dispose];
...
//# sourceMappingURL=toolProviderFactory.test.js.map