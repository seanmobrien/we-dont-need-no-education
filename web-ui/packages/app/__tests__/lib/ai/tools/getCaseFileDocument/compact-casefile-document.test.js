...
describe('compactCaseFileDocument', () => {
    it('removes null and undefined fields recursively', () => {
        ...
        expect(result.docProps && result.docProps[0] && result.docProps[0].doc).not.toHaveProperty('baz');
        expect(result.docProps && result.docProps.length).toBe(1);
        expect(result.docRel_targetDoc && result.docRel_targetDoc[0]).not.toHaveProperty('description', null);
        expect(result.docRel_sourceDoc && result.docRel_sourceDoc[0]).not.toHaveProperty('description', undefined);
        expect(result.email &&
            result.email.emailAttachments &&
            result.email.emailAttachments[0] &&
            result.email.emailAttachments[0].docs &&
            result.email.emailAttachments[0].docs[0]).not.toHaveProperty('content');
    });
    ...
    it('handles CTA and CTA response normalization', () => {
        ...
        const result = compactCaseFileDocument(doc);
        const ctaDoc = result.docProp && result.docProp.doc;
        expect(ctaDoc && ctaDoc.cta).toBeDefined();
        expect(ctaDoc &&
            ctaDoc.response &&
            ctaDoc.response.ctas &&
            ctaDoc.response.ctas.length).toBe(2);
        expect(ctaDoc &&
            ctaDoc.response &&
            ctaDoc.response.ctas &&
            ctaDoc.response.ctas[0].cta).not.toHaveProperty('baz');
    });
...
//# sourceMappingURL=compact-casefile-document.test.js.map