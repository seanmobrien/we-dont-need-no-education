...
describe('InMemoryStorageStrategy', () => {
    ...
    it('should store and retrieve individual todos', async () => {
        await storage.upsertTodoList(listFactory({
            id: idPrefix + 'list-1',
        }));
        const todo = {
            id: idPrefix + 'todo-1',
    ...
    it('should get todo to list mapping', async () => {
        const list = await storage.upsertTodoList(listFactory({
            id: idPrefix + 'list-1',
            title: 'Test List',
        }));
        const todo = {
...
describe('RedisStorageStrategy', () => {
    ...
    it('should store and retrieve individual todos', async () => {
        const list = await storage.upsertTodoList(redisListFactory({
            id: idPrefix + 'list-1',
        }));
        const todo = {
            id: idPrefix + 'todo-1',
    ...
    it('should retrieve all todos across lists', async () => {
        ...
        expect(incompleteTodos.some((t) => t.id === idPrefix + 'todo-1')).toBe(true);
    });
    const redisListFactory = (copy) => {
        return {
            ...copy,
