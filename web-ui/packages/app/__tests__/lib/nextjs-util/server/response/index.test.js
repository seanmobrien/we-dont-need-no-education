import { FetchResponse, makeResponse, makeJsonResponse, makeStreamResponse, } from '@/lib/nextjs-util/server/response';
global.Blob = class MockBlob {
    ...
    async text() {
        return new TextDecoder().decode(Buffer.concat(this.content.map((c) => Buffer.from(c))));
    }
};
...
describe('FetchResponse', () => {
    ...
    describe('ok()', () => {
        ...
        it('should return false for non-2xx status codes', () => {
            expect(new FetchResponse(Buffer.from(''), { status: 199 }).ok).toBe(false);
            expect(new FetchResponse(Buffer.from(''), { status: 300 }).ok).toBe(false);
            expect(new FetchResponse(Buffer.from(''), { status: 404 }).ok).toBe(false);
            expect(new FetchResponse(Buffer.from(''), { status: 500 }).ok).toBe(false);
        });
    });
    describe('text()', () => {
...
describe('makeJsonResponse', () => {
    ...
    describe('NextResponse.json() compatibility', () => {
        it('should mirror NextResponse.json() signature', () => {
            ...
            const response2 = makeJsonResponse({ error: 'Not found' }, { status: 404, headers: { 'X-Request-Id': '123' } });
            expect(response2.status).toBe(404);
            expect(response2.headers.get('X-Request-Id')).toBe('123');
        });
        it('should work in place of NextResponse.json()', async () => {
            const successResponse = makeJsonResponse({ success: true, data: { id: 1 } }, { status: 200 });
            expect(await successResponse.json()).toEqual({
                success: true,
                data: { id: 1 },
            });
            const errorResponse = makeJsonResponse({ success: false, error: 'Validation failed' }, { status: 400 });
...
describe('FetchResponse with ReadableStream', () => {
    it('should accept ReadableStream in constructor', async () => {
        ...
        const response = new FetchResponse(stream);
        expect(response.body).toBeInstanceOf(ReadableStream);
        expect(response.streamBody).toBe(stream);
        const text = await response.text();
