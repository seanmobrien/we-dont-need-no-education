import { wrapRouteRequest, EnableOnBuild, buildFallbackGrid, createInstrumentedSpan, reportEvent, } from '@/lib/nextjs-util/server/utils';
...
jest.mock('@/lib/nextjs-util/server/error-response', () => ({
    errorResponseFactory: (message, options = {}) => {
...
describe('Server Utils', () => {
    describe('Constants', () => {
        test('EnableOnBuild is a unique symbol', () => {
            ...
            expect(EnableOnBuild).toBe(EnableOnBuild);
        });
    ...
    describe('createInstrumentedSpan', () => {
        ...
        beforeEach(() => {
            ...
            otelContext.active.mockReturnValue(mockContext);
            trace.setSpan.mockReturnValue(mockContext);
            mockTracer.startSpan.mockReturnValue(mockSpan);
            otelContext.with.mockImplementation(async (ctx, fn) => fn(mockSpan));
        });
        test('creates span with default tracer name', async () => {
            ...
            expect(mockTracer.startSpan).toHaveBeenCalledWith('test-span', undefined, mockContext);
        ...
        test('handles function errors', async () => {
            ...
            LoggedError.isTurtlesAllTheWayDownBaby.mockReturnValue({
                message: 'Test error',
                name: 'Error',
                stack: 'stack trace',
            });
            ...
            await expect(result.executeWithContext(testFunction)).rejects.toThrow('Test error');
        ...
        test('returns no-op implementation when OpenTelemetry unavailable', async () => {
            trace.getTracer.mockImplementation(() => {
            ...
            const executionResult = await result.executeWithContext(async () => 'success');
            expect(executionResult).toBe('success');
        });
    ...
    describe('wrapRouteRequest', () => {
        ...
        beforeEach(() => {
            trace.getTracer.mockReturnValue(mockTracer);
            mockTracer.startActiveSpan.mockImplementation(async (name, options, parentCtx, fn) => {
                return await (fn ?? options)(mockSpan);
            });
            otelContext.active.mockReturnValue({});
            propagation.extract.mockReturnValue({});
        });
        test('wraps handler successfully', async () => {
            const handler = wrapRouteRequest(async (_req) => {
        ...
        describe('build fallback scenarios', () => {
            ...
            test('handles build fallback when IS_BUILDING is set', async () => {
                ...
                const handler = wrapRouteRequest(async (req) => {
        ...
        test('handles custom error callback', async () => {
            ...
            expect(LoggedError.isLoggedError).toBeDefined();
        });
        ...
        test('extracts route parameters from context', async () => {
            const handler = wrapRouteRequest(async (_req, _context) => {
                return new Response('success');
            });
            const mockRequest = {
                url: 'https://example.com/test',
            ...
            expect(mockTracer.startActiveSpan).toHaveBeenCalledWith('route.request', expect.objectContaining({
                attributes: expect.objectContaining({
                    'route.params': expect.stringContaining('{"id":"123","slug":"test"}'),
                }),
