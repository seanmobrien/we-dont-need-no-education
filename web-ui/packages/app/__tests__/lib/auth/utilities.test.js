...
describe('decodeToken', () => {
    ...
    describe('Decode without verification', () => {
        it('should decode token without verification when verify=false', async () => {
            ...
            decodeJwt.mockReturnValue(mockPayload);
        ...
        it('should handle decoding errors gracefully', async () => {
            ...
            await expect(decodeToken({ token: 'invalid-token', verify: false })).rejects.toThrow('Invalid token format');
        });
    });
    describe('Decode with verification', () => {
        ...
        it('should use custom issuer when provided', async () => {
            ...
            const mockPayload = { sub: '1234', iss: customIssuer };
            const mockJwks = { keys: [] };
            createRemoteJWKSet.mockReturnValue(mockJwks);
            jwtVerify.mockResolvedValue({
                payload: mockPayload,
            ...
            expect(createRemoteJWKSet).toHaveBeenCalledWith(expect.any(URL));
            const jwksUrl = createRemoteJWKSet.mock.calls[0][0];
        ...
        it('should handle JWKS fetch errors', async () => {
            createRemoteJWKSet.mockImplementation(() => {
