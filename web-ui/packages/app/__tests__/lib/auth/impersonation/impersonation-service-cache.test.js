...
describe('ImpersonationServiceCache', () => {
    ...
    beforeEach(() => {
        ImpersonationServiceCache.instance = null;
    ...
    describe('getOrCreate', () => {
        it('should create and cache a new impersonation service for first time use', async () => {
            ...
            const service = await cache.getOrCreate(userId, audience, mockServiceFactory);
        ...
        it('should return cached service for same user and audience', async () => {
            const userId = 'user1';
            const audience = 'test-audience';
            const service1 = await cache.getOrCreate(userId, audience, mockServiceFactory);
            const service2 = await cache.getOrCreate(userId, audience, mockServiceFactory);
            expect(mockServiceFactory).toHaveBeenCalledTimes(1);
        ...
        it('should create different instances for different audiences', async () => {
            ...
            mockServiceFactory
                ...
                .mockResolvedValueOnce(mockImpersonationService());
            const service1 = await cache.getOrCreate(userId, audience1, mockServiceFactory);
            const service2 = await cache.getOrCreate(userId, audience2, mockServiceFactory);
        ...
        it('should create different instances for different users', async () => {
            ...
            mockServiceFactory
                .mockResolvedValueOnce(mockImpersonationService())
                .mockResolvedValueOnce(mockImpersonationService());
            const service1 = await cache.getOrCreate(userId1, audience, mockServiceFactory);
            const service2 = await cache.getOrCreate(userId2, audience, mockServiceFactory);
            expect(mockServiceFactory).toHaveBeenCalledTimes(2);
            expect(service1).not.toBe(service2);
        });
...
//# sourceMappingURL=impersonation-service-cache.test.js.map