...
import { buildDrizzleAttachmentOrEmailFilter, buildDrizzleItemFilter, buildDrizzleQueryFilter, } from '@/lib/components/mui/data-grid/queryHelpers/drizzle/buildDrizzleFilter';
...
const createMockColumn = (name, dataType = 'string') => {
    const mockColumn = {
        name,
        type: 'column',
...
const createMockQuery = () => {
    ...
    query.where.mockImplementation((condition) => {
        ...
        if (condition && !condition.queryChunks) {
            condition.queryChunks = ['mocked condition'];
        }
...
const mockColumns = {
    ...
    emailId: createMockColumn('email_id', 'string'),
    documentId: createMockColumn('document_id', 'number'),
    ...
    describe('Basic functionality', () => {
        it('should return undefined when email_id is not provided', () => {
            const result = buildDrizzleAttachmentOrEmailFilter({
                attachments: true,
                email_id: undefined,
                email_id_column: mockColumns.emailId,
                document_id_column: mockColumns.documentId,
            });
...
describe('buildDrizzleItemFilter', () => {
    ...
    describe('Error handling', () => {
        it('should return undefined and log warning for unknown columns', () => {
            ...
            expect(result).toBeUndefined();
            expect(mockLogger.warn).toHaveBeenCalledWith("buildDrizzleItemFilter: Unknown column 'unknown_field' (mapped from 'unknown_field')");
        });
...
describe('buildDrizzleQueryFilter', () => {
    ...
    describe('URL and string sources', () => {
        it('should parse filter from URL string', () => {
            const urlString = 'https://example.com/api/data?filter=' +
        ...
        it('should parse filter from URL object', () => {
            ...
            const url = new URL('https://example.com/api/data?filter=' +
    ...
    describe('NextRequest source', () => {
        it('should parse filter from NextRequest', () => {
            ...
            const request = new NextRequest('https://example.com/api/data?filter=' +
                encodeURIComponent(JSON.stringify(filterModel)));
            buildDrizzleQueryFilter({
                query: mockQuery,
                source: request,
...
describe('Integration tests', () => {
    ...
    it('should work end-to-end with realistic data', () => {
        ...
        const getColumn = (name) => userTable[name];
        const columnMap = {
            name: 'firstName',
            created: 'createdAt',
            modified: 'updatedAt',
...
//# sourceMappingURL=buildDrizzleFilter.test.js.map