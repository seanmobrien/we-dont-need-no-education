...
import { getMultipleCaseFileDocuments, toolCallbackArrayResultSchemaFactory, } from '@/lib/ai/tools';
import z from 'zod';
...
export const GET = wrapRouteRequest(async (req) => {
    ...
    try {
        if (!!id) {
            const caseAccess = await checkCaseFileAuthorization(req, id, {
        ...
        return NextResponse.json({ error: error?.message ?? 'Unknown error', data: { error } }, { status: 500 });
    }
});
