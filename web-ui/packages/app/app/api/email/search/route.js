...
export const GET = wrapRouteRequest(async (req) => {
    try {
        ...
        const whereClauses = [];
        if (queryParam) {
            whereClauses.push("(e.subject ILIKE '%' || $1 || '%' OR sender.name ILIKE '%' || $1 || '%')");
        }
      ...
      ORDER BY e.sent_timestamp DESC \
        LIMIT ' +
            ...
            offset +
            ';';
        const result = await query((sql) => sql(querySql.toString(), [queryParam]), { transform: mapRecordToSummary });
        const countQuery = 'SELECT COUNT(*) AS records \
      FROM emails e \
      JOIN contacts sender ON e.sender_id = sender.contact_id \
      LEFT JOIN email_recipients er ON e.email_id = er.email_id \
      LEFT JOIN contacts recipient ON er.recipient_id = recipient.contact_id ' +
            (whereClauses.length > 0 ? 'WHERE ' + whereClauses.join(' AND ') : '') +
            ';';
        const total = await query((q) => q(countQuery, [queryParam]));
