...
import { TimelineAgentFactory, ServerTimelineAgent as TimelineAgent, } from '@/lib/ai/agents/timeline/agent-server';
...
export const POST = wrapRouteRequest(async (request) => {
    try {
        const { action, initialDocumentId, snapshot, propertyId } = await request.json();
        switch (action) {
            case 'initialize': {
                if (!initialDocumentId) {
                    return NextResponse.json({ error: 'initialDocumentId is required' }, { status: 400 });
                }
            ...
            case 'restore': {
                if (!snapshot) {
                    return NextResponse.json({ error: 'snapshot is required for restore action' }, { status: 400 });
                }
                try {
                    ...
                    return NextResponse.json({ error: `Failed to restore agent: ${error}` }, { status: 400 });
                }
            }
            case 'process': {
                return NextResponse.json({
                    ...
                    data: {
                        ...
                        additionalDocuments: Math.random() > 0.7 ? ['additional-doc-' + Date.now()] : [],
                    },
                });
            }
            default:
...
export const GET = wrapRouteRequest(async (request) => {
    ...
    const mockDocument = {
        ...
        metadata: {
            ...
            dateSent: new Date(Date.now() - Math.random() * 30 * 24 * 60 * 60 * 1000).toISOString(),
