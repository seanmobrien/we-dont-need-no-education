...
const STORED_MAP_KEY = Symbol.for('@no-education/typescript/SingletonProvider/StrongReferenceStorage/GlobalMap');
export class StrongReferenceStorage {
    ...
    get(key) {
        if (!Symbol.keyFor(key)) {
            log((l) => l.warn(`StrongReferenceStorage.get called with non-global symbol key: ${String(key)}\n${getStackTrace({ skip: 2, myCodeOnly: true })}`));
        }
