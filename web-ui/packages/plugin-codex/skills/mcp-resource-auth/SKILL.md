...
The OAuth wrapper adds two helper tools alongside tools exposed by the child MCP server:

- `mcp_resource_auth_list_abilities`: returns tool names, descriptions, input schemas, resource count, and resource template count.
- `mcp_resource_auth_list_resources`: returns a directory-style resource listing grouped by URI path. If the child server does not support MCP resources, report that no concrete resources or templates are exposed.
- `mcp_resource_auth_manage_auth`: auth helper with `status`, `login`, and `clear-cache` actions. `status` validates bearer auth against `[MCP Server]/api/auth/session` and reports either unauthenticated state or authenticated user/session/permission details.

When MCP resources are available, use this order:

1. `list_mcp_resources` to discover concrete resources.
2. `list_mcp_resource_templates` when a parameterized lookup is needed.
3. `read_mcp_resource` for the selected resource URI.

When MCP tools are available, inspect tool descriptions and input schemas before calling them. Prefer read-only operations unless the user explicitly asks for a write or mutation.
