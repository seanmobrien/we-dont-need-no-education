import { cryptoRandomBytes } from '../../nextjs/crypto-random-bytes';
...
export const splitIds = (id: string): [string, string | undefined] => {
  if (!id) {
    // No ID provided to splitIds, returning empty values.
    return ['', undefined];
  }
  const splitIndex = id.indexOf(':');
  if (splitIndex === -1) {
    // No ":" found in ID, returning as is.
    return [id, undefined];
  }
  if (splitIndex === 0 || splitIndex === id.length - 1) {
    // Invalid ID format, returning empty values.
    return ['', undefined];
  }
