import { RedisClientType } from '@compliance-theater/redis';

// Storage for mock data
const mockRedisData = new Map<string, string>();
const mockSortedSetData = new Map<string, Map<string, number>>();

const MOCK_CLIENT = Symbol.for('@we-dont-need-no-education/redis-mock-client');

const createMockClient = (): RedisClientType => {
    let withJestTestExtensions: (() => { singletonStore: Map<symbol, any> });
    try {
        withJestTestExtensions = jest.requireActual('../../__tests__/shared/jest.test-extensions').withJestTestExtensions;
    } catch {
        withJestTestExtensions = jest.requireActual('../__tests__/jest.test-extensions').withJestTestExtensions;
    }
    let client: RedisClientType = withJestTestExtensions().singletonStore.get(MOCK_CLIENT) as RedisClientType;
    if (!client) {
        client = {
        ...
        withJestTestExtensions().singletonStore.set(MOCK_CLIENT, client);
    }
    return client;
}

export const createClient = jest.fn((arg?: any) => {
    if (arg === 'teardown') {
        mockRedisData.clear();
        mockSortedSetData.clear();
