  ...
  useEffect(() => {
    const fetchRelatedResponses = async () => {
      try {
        ...
        const related = result.results?.filter((response: CallToActionResponseDetails) => 
    ...
    <>
      ...
      <Grid container spacing={2}>
        <Grid item xs={6} md={3}>
          ...
          <Typography variant="h6">{formatScore(row.compliance_rating)}</Typography>
        </Grid>
        <Grid item xs={6} md={3}>
          <Typography variant="subtitle1" fontWeight="bold">Severity</Typography>
          <Typography variant="h6">{formatScore(row.severity)}</Typography>
        </Grid>
        <Grid item xs={6} md={3}>
          <Typography variant="subtitle1" fontWeight="bold">Sentiment</Typography>
          <Typography variant="h6">{formatScore(row.sentiment)}</Typography>
        </Grid>
        <Grid item xs={6} md={3}>
          <Typography variant="subtitle1" fontWeight="bold">Title IX</Typography>
          <Typography variant="h6">{formatScore(row.title_ix_applicable)}</Typography>
        </Grid>
      </Grid>
          ...
          {relatedResponses.length > 0 ? (
            <Stack spacing={1}>
              {relatedResponses.map((response, index) => (
                <Box key={response.propertyId} sx={{ p: 2, border: '1px solid #e0e0e0', borderRadius: 1 }}>
        ...
        <AccordionDetails>
          <Stack spacing={2}>
            {row.reasonable_reasons?.length && (
              <Box>
                <Typography variant="subtitle1" fontWeight="bold">Reasonable Request Reasons</Typography>
                <Stack direction="row" spacing={1} flexWrap="wrap">
