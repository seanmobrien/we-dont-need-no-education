...
declare module 'lib/react-util/errors/logged-error' {
  /**
   ...
   *
   * @internal
   * @interface TurtleRecursionParams
   *
   ...
   *
   * @property {boolean} [relog=false] - Whether to re-log the error if it has already
   *   been logged.
   *   By default, a LoggedError will not be re-logged. If this is set to true, it will
   *   re-log the error with the same context as the original log. This can be useful for
   *   providing additional context in subsequent logs.
   *
   ...
   * @example
   * ```typescript
   * const params: TurtleRecursionParams = {
   *   log: true,
  ...
  export class LoggedError implements Error {
     ...
     *
     * @param {TurtleRecursionParams} [options] - Configuration options for error handling - see {@link TurtleRecursionParams}.
    ...
    /**
     * Symbol.toStringTag implementation to support better debugging and diagnostic logging.
     *
    ...
    /**
     * The cause of the underlying error (if any).
     */
    get cause(): unknown;

    /**
     * Runtime type guard to detect LoggedError instances.
     * @param e - value to check
     */
    static isLoggedError(e: unknown): e is LoggedError;

    /**
     * Primary recursive wrapper and logger used across the codebase. This method
     * accepts arbitrary values and always returns a LoggedError instance. It never throws.
     *
     * @param e - any value to wrap/normalize
     * @param options - logging/reporting configuration (see TurtleRecursionParams)
     * @returns a LoggedError instance that wraps the input
     */
    static isTurtlesAllTheWayDownBaby(
      e: unknown,
    ...
    /**
     * Utility to build a human readable message from arbitrary inputs.
     *
     * @param options - any value (error, object, primitive)
