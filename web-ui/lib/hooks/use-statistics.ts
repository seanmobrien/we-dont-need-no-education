 ...
 */
const fetchModelStatistics = async (source: 'database' | 'redis' = 'database'): Promise<ModelStat[]> => {
 ...
 */
const fetchQueueStatistics = async (): Promise<{ summary: QueueSummary; queues: QueueInfo[] }> => {
  const response = await fetch('/api/statistics/queues');
  
  if (!response.ok) {
    throw new Error('Failed to fetch queue statistics');
  }
  
  const data: ApiResponse<{ summary: QueueSummary; queues: QueueInfo[] }> = await response.json();
 ...
 */
export const useModelStatistics = (source: 'database' | 'redis' = 'database') => {
  return useQuery({
 ...
 */
export const useStatistics = (modelSource: 'database' | 'redis' = 'database') => {
