...
Storage strategy is configured via the `todo_storage_strategy` feature flag in
`web-ui/lib/site-util/feature-flags/known-feature.ts`. Each backend also has a
dedicated configuration flag that accepts JSON:

- `todo_storage_in_memory_config`
- `todo_storage_redis_config`

Both flags can contain either an object or a JSON string with the
`StorageStrategyConfig` fields (`ttl`, `keyPrefix`, `enableFallback`).
...
2. Set the feature flags (via Flagsmith or environment):
...
3. Enable the `todo_storage_strategy` and storage config feature flags
