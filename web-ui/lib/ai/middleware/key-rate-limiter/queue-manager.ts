...
export class RateLimitQueueManager {
  private static instance: RateLimitQueueManager;
  
  static getInstance(): RateLimitQueueManager {
    if (!RateLimitQueueManager.instance) {
      RateLimitQueueManager.instance = new RateLimitQueueManager();
    }
    return RateLimitQueueManager.instance;
  }
  ...
  async dequeueRequests(generation: 1 | 2, modelClassification: string, maxCount: number = 10): Promise<RateLimitedRequest[]> {
    const redis = await getRedisClient();
  ...
  async getQueueSize(generation: 1 | 2, modelClassification: string): Promise<number> {
    const redis = await getRedisClient();
    const queueKey = this.getQueueKey(generation, modelClassification);
