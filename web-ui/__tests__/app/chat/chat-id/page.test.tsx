...
const unauthorizedMock = jest.fn(() => { throw new Error('UNAUTHORIZED'); });
...
const setSession = (userId: number | null) => {
  if (userId == null) {
    ...
    authMock.mockImplementation(() => Promise.resolve({ user: { id: userId } }));
  }
};

const drizDbWithInitMock = jest.fn();
jest.mock('@/lib/drizzle-db', () => ({
  drizDbWithInit: (cb?: (db: { query: { chats: { findFirst: typeof drizDbWithInitMock } } }) => unknown) =>
    cb ? cb({ query: { chats: { findFirst: drizDbWithInitMock } } }) : drizDbWithInitMock(),
}));

const isUserAuthorizedMock = jest.fn();
jest.mock('@/lib/site-util/auth', () => ({
  isUserAuthorized: (args: { signedInUserId: number; ownerUserId: number }) => isUserAuthorizedMock(args),
}));

jest.mock('@/lib/nextjs-util', () => ({
  extractParams: async (req: { params: Promise<{ chatId: string }> }) => ({ chatId: (await req.params).chatId }),
}));

jest.mock('@/components/email-message/dashboard-layout/email-dashboard-layout', () => ({
  EmailDashboardLayout: ({ children }: { children: React.ReactNode }) => <div data-testid="layout">{children}</div>,
}));

interface MockChatHistoryProps { chatId: string; title?: string }
...
type PossibleNode = React.ReactElement | React.ReactElement[] | null | undefined | string | number | boolean;
const findChatHistoryProps = (node: PossibleNode): MockChatHistoryProps | null => {
  if (!node) return null;
  ...
  if (typeof typeFn === 'function' && (typeFn as { name?: string }).name === 'ChatHistory') {
      return node.props as MockChatHistoryProps; // props shape controlled by our mock
    }
  const element = node as React.ReactElement & { props?: { children?: PossibleNode } };
  return findChatHistoryProps(element.props && element.props.children as PossibleNode);
  }
  return null;
};
...
describe('ChatDetailPage', () => {
  ...
  test('notFound when user unauthorized', async () => {
    setSession(42);
    drizDbWithInitMock.mockResolvedValueOnce({ id: 'abc123', userId: 99, title: 'Secret' });
  ...
  test('success passes title', async () => {
    setSession(42);
    drizDbWithInitMock.mockResolvedValueOnce({ id: 'abc123', userId: 42, title: 'My Chat' });
    isUserAuthorizedMock.mockResolvedValueOnce(true);
    const jsx = await ChatDetailPage({ params: Promise.resolve({ chatId: 'abc123' }) });
  ...
  test('success with null title passes undefined', async () => {
    setSession(7);
    drizDbWithInitMock.mockResolvedValueOnce({ id: 'zzz999', userId: 7, title: null });
    isUserAuthorizedMock.mockResolvedValueOnce(true);
  const jsx = await ChatDetailPage({ params: Promise.resolve({ chatId: 'zzz999' }) });
  const props = findChatHistoryProps(jsx);
...
describe('getChatDetails', () => {
  ...
  test('returns ok false when unauthorized', async () => {
    drizDbWithInitMock.mockResolvedValueOnce({ id: 'c1', userId: 2, title: 'T' });
    isUserAuthorizedMock.mockResolvedValueOnce(false);
  ...
  test('returns ok true with title', async () => {
    drizDbWithInitMock.mockResolvedValueOnce({ id: 'c1', userId: 5, title: 'T' });
    isUserAuthorizedMock.mockResolvedValueOnce(true);
  ...
  test('returns ok true with undefined title when null', async () => {
    drizDbWithInitMock.mockResolvedValueOnce({ id: 'c2', userId: 5, title: null });
    isUserAuthorizedMock.mockResolvedValueOnce(true);
