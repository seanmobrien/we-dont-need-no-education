...
describe('set-normalized-defaults utility functions', () => {
  // Mock the utilities to test the core logic
  const isJsonCodeBlock = (text: string): boolean => {
  ...
  const isValidJsonObject = (text: string): boolean => {
    const trimmed = text.trim();
  ...
  describe('extractJsonFromCodeBlock', () => {
    ...
    it('should handle whitespace correctly', () => {
      ...
      expect(extractJsonFromCodeBlock('```json{"key": "value"}```')).toBe(
        '{"key": "value"}',
      );
    });

    it('should handle edge cases', () => {
      // Multiple ``` occurrences - should stop at first one
      expect(
        extractJsonFromCodeBlock('```json\n{"key": "```"}\n``` more ``` text'),
      ).toBe('{"key": "```"}');

      throws(() => extractJsonFromCodeBlock('json\n{"key": "value"}\n```'));
      // No proper start - should return original
    });
  });
  ...
  describe('isValidJsonObject', () => {
    it('should correctly identify valid JSON objects', () => {
      ...
      expect(isValidJsonObject('{"number": 123, "array": [1, 2, 3]}')).toBe(
        true,
      );
  ...
  describe('integration scenarios', () => {
    ...
    it('should handle non-JSON content correctly', () => {
      ...
      // These functions should still work even with non-JSON input
  ...
  describe('middleware configuration', () => {
    ...
    it('should validate middleware structure', async () => {
      ...
      expect(typeof setNormalizedDefaultsMiddleware.transformParams).toBe(
        'function',
      );
      expect(typeof setNormalizedDefaultsMiddleware.wrapGenerate).toBe(
        'function',
      );
      expect(typeof setNormalizedDefaultsMiddleware.wrapStream).toBe(
        'function',
      );
    });
  });
