...
describe('Message Structure Preservation Interface', () => {
  describe('MessageStructureOptions Interface', () => {
    ...
    it('should merge partial options with defaults', () => {
      ...
      expect(customOptions.partRules?.toolResult).toBe(true); // Default
      expect(customOptions.debug).toBe(true);
      expect(customOptions.performance?.enableCaching).toBe(true); // Default
    });
    ...
    it('should validate deep merging of nested options', () => {
      ...
      expect(options.metadata?.messageIds).toBe(true); // Default preserved
      expect(options.performance?.enableCaching).toBe(false);
      expect(options.performance?.cacheTtlMs).toBe(300000); // Default preserved
    });
  });
  ...
  describe('Type Guards', () => {
    ...
    it('should check if preservation is enabled', () => {
      ...
      expect(isPreservationEnabled({})).toBe(true); // Default to true
    });
  });
...
describe('Message Structure Preservation Utilities', () => {
  ...
  describe('Contextual Preservation', () => {
    it('should preserve recent interactions', () => {
      ...
      expect(result.preserved).toHaveLength(3); // Last 3 messages
  ...
  describe('Error Handling', () => {
    it('should handle invalid messages gracefully', () => {
      const invalidMessage: UIMessage = {
        id: '1',
        role: 'user' as const,
        parts: null as any, // Invalid parts
      };
