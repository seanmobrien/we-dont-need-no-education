...
describe('Message Optimizer Tools', () => {
  ...
  describe('Message Optimization', () => {
    ...
    it('should preserve recent messages when no optimization is needed', async () => {
      ...
      await optimizeMessagesWithToolSummarization(toolMessages, 'gpt-4', 'test_user');
      ...
      const messages: UIMessage[] = [
        ...
        createToolMessage('call_1', 'search', { query: 'test' }, 'Found results'),
      ...
      const messages: UIMessage[] = [
        ...
        createToolMessage('old_call', 'search', { query: 'old' }, 'Old results'),
        ...
        createToolMessage('recent_call', 'search', { query: 'recent' }, 'Recent results'),
        createAssistantMessage('Recent response', 'assistant_4'),
      ...
      const recentToolCalls = optimized.filter(m => 
        hasToolCalls(m) && extractToolCallIds(m).includes('recent_call')
      );
      ...
      const oldToolCalls = optimized.filter(m => 
        hasToolCalls(m) && m.toolInvocations?.some(inv => 
          inv.toolCallId === 'old_call' && 
          inv.state === 'result' &&
          !('result' in inv && typeof inv.result === 'string' && inv.result.includes('[SUMMARIZED'))
        )
      );
  ...
  describe('Edge Cases', () => {
    it('should handle empty message array', async () => {
      const optimized = await optimizeMessagesWithToolSummarization([], 'gpt-4', 'test_user');
    ...
    it('should log optimization metrics', async () => {
      const largeMessages: UIMessage[] = [
        ...Array.from({ length: 20 }, (_, i) => ({
          ...
          parts: [{ type: 'text' as const, text: `User message ${i} with lots of content that makes the message quite large and contributes to token usage` }],
          id: `user_${i}`,
          createdAt: new Date(),
        })),
        ...Array.from({ length: 20 }, (_, i) => ({
          ...
          parts: [{ type: 'text' as const, text: `Assistant response ${i} with detailed explanation and comprehensive information` }],
          id: `assistant_${i}`,
          createdAt: new Date(),
        })),
      ];
