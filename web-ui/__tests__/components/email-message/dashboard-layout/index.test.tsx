...
jest.mock('@/components/email-message/dashboard-layout/email-dashboard-layout', () => {
  const MockEmailDashboardLayout = React.forwardRef<HTMLDivElement, { children: React.ReactNode; session: unknown }>((props, ref) => (
    <div ref={ref} data-testid="email-dashboard-layout">{props.children}</div>
  ));
...
jest.mock('@/components/email-message/dashboard-layout/custom-email-page-item', () => {
  const MockCustomEmailPageItem = React.forwardRef<HTMLDivElement, { item: unknown; mini: boolean; emailId: string }>((props, ref) => (
    <div ref={ref} data-testid="custom-email-page-item" />
  ));
...
jest.mock('@/components/email-message/dashboard-layout/email-dashboard-toolbar-action', () => {
  const MockEmailDashboardToolbarAction = React.forwardRef<HTMLDivElement>((props, ref) => (
    <div ref={ref} data-testid="email-dashboard-toolbar-action" />
  ));
  MockEmailDashboardToolbarAction.displayName = 'MockEmailDashboardToolbarAction';
...
describe('Dashboard Layout Module Exports', () => {
  describe('Component Exports', () => {
    ...
    it('should export EmailDashboardToolbarAction component', () => {
      ...
      expect(typeof DashboardLayoutModule.EmailDashboardToolbarAction).toBe('object');
    });
  });
  ...
  describe('Configuration Exports', () => {
    ...
    it('should export Branding with correct structure', () => {
      expect(DashboardLayoutModule.Branding.title).toBe('Mystery Compliance Theater 2000');
      expect(React.isValidElement(DashboardLayoutModule.Branding.logo)).toBe(true);
    });
  });
  ...
  describe('All Expected Exports', () => {
    ...
    it('should not export any unexpected items', () => {
      ...
      const moduleAsRecord = DashboardLayoutModule as Record<string, unknown>;
      const runtimeExports = moduleKeys.filter(key => 
        typeof moduleAsRecord[key] !== 'undefined'
      );
  ...
  describe('Component Props Interface', () => {
    it('should have components that accept the expected props', () => {
      const { EmailDashboardLayout, CustomEmailPageItem, EmailDashboardToolbarAction } = DashboardLayoutModule;
  ...
  describe('TypeScript Compatibility', () => {
    it('should have components that are TypeScript compatible', () => {
      ...
      const components = {
        ...
        EmailDashboardToolbarAction: DashboardLayoutModule.EmailDashboardToolbarAction,
      };
