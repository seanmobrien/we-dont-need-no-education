...
test.describe('Error Handling', () => {
  ...
  test('should handle invalid URLs gracefully', async ({ page }) => {
    ...
    for (const url of invalidUrls) {
      ...
      const unhandledError = page.locator('text=unhandled, text=crashed, text=something went wrong');
      const hasUnhandledError = await unhandledError.isVisible({ timeout: 2000 });
      ...
      const errorHandled = currentUrl.includes('/404') || 
                          currentUrl !== url ||
                          await page.locator('text=404, text=not found, text=error').isVisible();
  ...
  test('should handle browser compatibility issues', async ({ page }) => {
    // Test basic functionality that might break in older browsers
    await page.goto('/');
    
    // Check that modern JavaScript features are handled
    const hasConsoleErrors = await page.evaluate(() => {
      try {
        // Test modern features
        const testArray = [1, 2, 3];
        const [first, ...rest] = testArray; // Destructuring
        const result = testArray.map(x => x * 2); // Arrow functions
        return false;
      } catch (error) {
    ...
    console.log('Page handles browser compatibility appropriately');
  });
});