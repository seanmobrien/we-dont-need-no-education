...
import { signInWithCredentials, signOut, testUsers, isSignedIn } from '../helpers/auth-helper';
...
test.describe('Authentication', () => {
  ...
  test('should load sign-in page', async ({ page }) => {
    ...
    const signInContent = page.locator('h1, h2, title').filter({ hasText: /sign in|login/i });
  ...
  test('should handle credentials sign-in flow', async ({ page }) => {
    ...
    const credentialsOption = page.locator('button:has-text("Credentials"), a:has-text("Credentials"), form').first();
    
    if (await credentialsOption.isVisible({ timeout: 5000 })) {
      // If it's a button/link, click it
      if (await credentialsOption.evaluate(el => el.tagName.toLowerCase() === 'button' || el.tagName.toLowerCase() === 'a')) {
      ...
      if (await emailField.isVisible({ timeout: 5000 }) && await passwordField.isVisible({ timeout: 5000 })) {
  ...
  test('should handle authentication errors gracefully', async ({ page }) => {
    ...
    if (await emailField.isVisible({ timeout: 5000 }) && await passwordField.isVisible()) {
